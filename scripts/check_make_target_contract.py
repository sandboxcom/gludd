...
def validate_contract(makefile_path: Path, contract: dict[str, Any]) -> list[str]:
    ...
    for entry in contract["targets"]:
        ...
        if stanza is None:
            ...
            continue
        if not any(name in line for line in help_lines):
        ...
        for environment_variable in entry.get("environment_variables", []):
            if not isinstance(environment_variable, str) or (
                ...
                errors.append(
                    f"{name}: behavior does not demonstrate {environment_variable}"
                )
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        ...
        return 1
    print(f"make-target-contract: PASS ({len(contract['targets'])} targets)")
    return 0
