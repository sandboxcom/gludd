...
def _batch_recreate_mode() -> Literal["auto", "always"]:
    """Rebuild tables only on SQLite, which cannot alter constraints natively."""
    return "always" if op.get_bind().dialect.name == "sqlite" else "auto"


def _namespace_unique_constraint_name() -> str:
    """Return the reflected SQLite or server-generated PostgreSQL name."""
    if op.get_bind().dialect.name == "postgresql":
        return "variable_namespaces_namespace_key"
    return "uq_variable_namespaces_namespace"


def upgrade() -> None:
    recreate = _batch_recreate_mode()
    op.create_table(
        "projects",
    ...
    # rebuilt twice). PostgreSQL must keep native ALTER TABLE semantics because
    # recreating a referenced table would require dropping dependent FKs.
    op.add_column("todos", sa.Column("project_id", sa.String(32), nullable=True))
    op.create_index("ix_todos_project_id", "todos", ["project_id"])
    with op.batch_alter_table("todos", recreate=recreate) as batch_op:
    ...
    with op.batch_alter_table("todo_events", recreate=recreate) as batch_op:
        batch_op.create_foreign_key(
    ...
    with op.batch_alter_table("task_returns", recreate=recreate) as batch_op:
        batch_op.create_foreign_key(
    ...
    with op.batch_alter_table("task_decisions", recreate=recreate) as batch_op:
        batch_op.create_foreign_key(
    ...
    with op.batch_alter_table("queues", recreate=recreate) as batch_op:
        batch_op.create_foreign_key(
    ...
    with op.batch_alter_table("audit_events", recreate=recreate) as batch_op:
        batch_op.create_foreign_key(
    ...
    # PostgreSQL instead assigns ``variable_namespaces_namespace_key`` in 001,
    # so the name is selected by dialect. (The original migration tried to drop
    ...
    with op.batch_alter_table(
        ...
        batch_op.drop_constraint(_namespace_unique_constraint_name(), type_="unique")
    ...
    with op.batch_alter_table("bucket_leases", recreate=recreate) as batch_op:
        batch_op.create_foreign_key(
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("variable_namespaces", recreate=recreate) as batch_op:
