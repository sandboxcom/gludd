...
- [x] T-BETA3-RELEASE-SMOKE-MATRIX — Add credential-safe Azure/RunPod, OpenCode boot, and release-readiness smoke coverage | paths: `tests/e2e/test_release_smoke_matrix.py`, `docs/RELEASE_SMOKE_MATRIX.md` | priority: high | effort: S | status: completed | evidence: `make test-files TESTFILES='tests/e2e/test_release_smoke_matrix.py'` 9 passed; `make lint-files FILES='tests/e2e/test_release_smoke_matrix.py'` passed; dry-run network isolation and fail-closed live credential tests
...
- [x] T-BETA3-CONNECTOR-BATCH5-DOCS — Document Redfish/SNMP, host-reader, ingest-format, and namespaced resource compatibility contracts for the batch-5 E2E shard | paths: `docs/audit/CONNECTOR_BATCH5_COMPATIBILITY.md` | priority: medium | effort: S | status: completed | evidence: docs/audit/CONNECTOR_BATCH5_COMPATIBILITY.md; community findings for OpenShift, Nomad, Podman, containerd, and parallel Compose namespace collisions; `make mcp-docs-check`; `make check-task-registration`; `make validate-task-ledger`
 - [ ] T-BETA3-CONNECTORS-BATCH5-ORCHESTRATION — Align OpenShift and Nomad E2E fixtures with namespace, transport injection, query-mode, and SSRF timing contracts | paths: `tests/e2e/test_connectors_batch5_workflows.py` | priority: high | effort: S | status: pending
...
- [x] T-CODEX-CONT-046 — Reclaim stale owners only after identity validation | priority: high | effort: M | status: completed | evidence: `scripts/reap_orphan_pytest.py` now validates namespaced gate/agent owner command identity before permitting orphan-tree cleanup and preserves descendants while a live owner exists; `tests/unit/test_orphan_pytest_reaper.py` (6 passed, including stale status PID and live-owner regressions, 2026-07-26)
...
<!-- T-BETA3-OPENCode additional path registration: tests/e2e/test_opencode_plugin_load.py tests/e2e/test_opencode_boot_e2e.py -->
