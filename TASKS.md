...
- [ ] S83.5 — **Short namespaced integration temp paths**: keep macOS AF_UNIX socket paths below the platform limit while preserving per-project isolation and parallel-safe cleanup. | evidence: beta4 preflight gate reproduced 2 `AF_UNIX path too long` failures in Firecracker REST integration coverage; failing-first repair in progress | priority: high | effort: S | status: in_progress
- [ ] S83.6 — **Integration fail-closed compatibility repairs**: preserve safe model-registry policy for mocked provider downloads and make rootfs destination cleanup resilient to concurrent replacement without weakening integration assertions. | evidence: beta4 preflight gate reproduced 3 model-registry 404s and 1 `ENOTEMPTY` rootfs cleanup failure; failing-first repair in progress | priority: high | effort: M | status: in_progress
- [ ] S83.7 — **Accurate integration-health failure accounting**: parse xdist pytest failure output into exact failed tests/files so the observable gate summary never reports “0 failed files” when explicit failures occurred. | evidence: beta4 preflight gate printed 6 pytest failures but summarized `0 failed files, 1 total failures` and `unknown`; repair pending current coding wave | priority: high | effort: S | status: pending
