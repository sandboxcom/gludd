...
- [ ] **CGW-20** — security_backlog.py converted from an all-pass stub to a landed-guard static regression gate (`make security-backlog-gate`); D-07..D-30 IDs map to `docs/audit/NEW_FINDINGS_2026-06-16.md` (DISJOINT from TASKS.md Phase D's D-#N scheme — do not conflate). 3/24 items are real LANDED-VERIFIED probes (D-14 clone SSRF/RCE guard, D-18 audit-log wiring, D-27 RLIMIT sandbox caps); verified-OPEN residuals still needing a fix: D-07 (db/models.py CheckConstraint blob bounding), D-11 (no rate limiter on POST /api/todos creation), D-13 (no PRAGMA journal_size_limit on the WAL db), D-17 (no worker PSK rotation schedule). Report-only: scripts/backlog_audit.py references a nonexistent `make backlog-audit` target.
