...
- [ ] ENF.1 — Keep enforcement plugin imports process-pure so runtime checks cannot multiply detached, unobservable gate trees | paths: `.opencode/plugin/enforce-multitask.ts`, `.opencode/plugin/impl/enforce_stop_impl.ts`, `scripts/test_hook_runtime.py`, `tests/unit/test_enforcement_gate_spawn_safety.py`, `docs/features/ENFORCEMENT_PROCESS_ISOLATION.md` | evidence: two red-first structural runs independently exposed `spawnGateRefresh` in both enforcement factories; implementation removed plugin-owned child creation; all-source regression 1/1, focused process suite 12/12, enforcement runtime 124/124, Node compatibility 5/5, plugin manifest 99/99, task ledger/integrity, and 56,046-test collection passed; post-verification `make ps` reported no matching project processes; full repository gate pending | priority: high | effort: S | status: in_progress
