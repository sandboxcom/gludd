...
- [ ] RP.13 — Fix enforcement streak counter blocking legitimate git operations | priority: critical | effort: medium | status: pending | evidence: enforce-delegate.ts treats `make git-add` + `make git-commit` as 2 consecutive "grinding" mutations, hitting MAINTHREAD_THRESHOLD=2 and hard-denying the commit. Agent was forced to `make disengage-enforcement` 5+ times this session to commit critical fixes. Root cause: the streak counter does not distinguish between "grinding on routine inline edits" (bad) and "committing/shipping work" (good). Fix: add git-operation targets (git-add, git-commit, ship-commit, git-push-sandboxcom, git-tag-push, release-cut, batch-push) to an ALLOWLIST in enforce-delegate.ts that resets or skips the streak counter. These are terminal operations (commit/push/tag), not grinding. The streak counter should only count Read/Edit/Write/bash-test/bash-lint operations between dispatches, not git shipping operations. Structural test: verify that calling make git-commit after make git-add does NOT increment the streak counter or trigger a block. Spec: docs/specs/TBD_enforcement_stallowlist.md
