...
- [ ] A.4 — Cut v0.1.0-beta.2 release: `make release-cut` + verify-release-artifact | priority: high | effort: small | status: BLOCKED on CI (2026-07-15) | blocker: CI PENDING on development (run 29451816311, HEAD 9b8d78243dc1, `in_progress`); ci-verdict-safe cooldown active (check #35) — CI cooldown fix applied (9b8d7824) — prior cooldown masked actual RED state (run 29449765249, incident BUGS.md #13), now ci-verdict-safe records last-known-verdict to prevent masking; molecule failed_when Jinja2 fixes in CI; release-cut cannot proceed until CI verdict is GREEN on the development tip, then development→master merge + `make release-cut TAG=v0.1.0-beta.2` | history: re-opened 2026-07-14 audit — was ticked `[x]` while its own evidence string read "beta.2 SKIPPED — release-cut was started but beta.2 tag was not pushed." A release that was not cut is not a completed release-cut item. Decide explicitly: either cut beta.2 once CI is green, or close as SUPERSEDED by A.9 (v0.1.0-beta.1 shipped) with that rationale written here.
