...
- [ ] AZL.2 — Replace static work-item cost tiers with exact Azure Retail Prices meter selection and delayed billed-cost reconciliation | paths: `src/general_ludd/infra/azure_retail_pricing.py`, `src/general_ludd/infra/deploy_strategy.py`, `tests/unit/test_azure_retail_pricing.py`, `tests/unit/test_deploy_strategy.py`, `tests/e2e/providers/test_azure_retail_pricing_e2e.py`, `tests/e2e/providers/test_azure_provision_e2e.py`, `docs/research/AZURE_TERRAFORM_EVENT_ELASTICITY_E2E_EVIDENCE.md` | evidence: exact pre-deploy Container Apps GPU/vCPU/memory meter slice implemented and credential-free live East US meter contract passed; exact VM warmup, ancillary costs, delayed billed reconciliation, and calibrated cohort evidence remain pending | priority: high | effort: L | status: in_progress
