...
**Last consolidated: 2026-07-15 Session 37. NF.2 P3+P4 completed (46 tests, f68957fe+773f9275), NF.5 completed (36 tests), NF.7 P5 completed (24 tests), NF.3 4 roles fleshed (83 tests), NF.4 9/10 roles (269 tests), NF.6 8+ roles (88 tests), NF.9 282 tests. NF.1+NF.2+NF.5+NF.7+NF.8+NF.10 COMPLETED. HEAD: 773f9275 on development. 336 boxes total: 330 checked, 6 pending (A.4, C.18, 4 in-progress).**
...
| NF | New Features (v0.1.0-beta.2) | 3 | 10 | 70% |
...
- [x] NF.2 — Unikernel sandbox: P1 Firecracker/GVisor backends + P2 image builder + P3 VMSandboxManager + P4 real executor | spec: docs/specs/FEATURE_UNIKERNEL_SANDBOX.md | status: completed | evidence: P1+P2 done — Firecracker + GVisor backends (22 tests) + P2 image builder (48 tests) + P3 VMSandboxManager boot-dispatch-verify-release lifecycle + VMInstance state machine + VMMetrics observability (23 tests, commit f68957fe) + P4 real executor typecheck fix + agent_executor wiring (23 tests, commit 773f9275). 7 src files (vm/{firecracker_backend,gvisor_backend,image_builder,agent_executor,sandbox_manager,instance,metrics}.py), 116 tests pass. commits db2699da (P1), 62f1bab8 (P2), f68957fe (P3), 773f9275 (P4)
- [-] NF.3 — Binary RE collection: 8 roles + 3 knowledge modules | spec: docs/specs/FEATURE_BINARY_RE.md | status: in_progress (Phase A+B+C done: 288 tests, 4 roles fully fleshed out; molecule tests added) | evidence: 8 roles (cyberchef_transform, deobfuscate, frida_instrument, fuzz_target, gdb_analyze, ghidra_analyze, prompt_injection_scan, radare2_analyze), 3 module_utils (fuzzing_strategies, obfuscation_techniques, prompt_injection_detector). 4 roles fully fleshed out with report-only Python backends: gdb_analyze, radare2_analyze, ghidra_analyze (52 tests, commit 5684b4d6), frida_instrument (31 tests, commit acdc1285) = 83 tests for the 4 fleshed-out roles. 288 total binary_re tests pass across all test files. molecule/playbooks/binary_re/ added; commits db2699da (9-feature wave), 816d7be6 (molecule tests), 5684b4d6 (gdb/r2/ghidra), acdc1285 (frida)
- [-] NF.4 — Radio engineer collection: 10 roles + 5 knowledge modules | spec: docs/specs/FEATURE_RADIO_ENGINEER.md | status: in_progress (9/10 roles done: 269 tests, 9 roles fleshed out with standalone Python CLIs; 1 role remaining) | evidence: 10 roles (antenna_design, decode_digital, exam_quiz, link_budget, marine_decode, propagation_model, regulation_lookup, sdr_capture, signal_identify, spectrum_scan), 5 module_utils (antenna_types, frequency_allocations, modulation_schemes, propagation_models, radio_exam_data). 9/10 roles fleshed out: propagation_model + regulation_lookup + exam_quiz (55 tests, commit 18a8295a), link_budget (32 tests), remaining 5 roles from prior waves. 269 total radio tests pass; commits db2699da (9-feature wave), 816d7be6, 62f1bab8 (7 roles molecule tests), 18a8295a (propagation/regulation/exam_quiz)
...
- [-] NF.6 — OS expert collection: 12 roles + 5 knowledge modules + 6 connectors | spec: docs/specs/FEATURE_OS_EXPERT.md | status: in_progress (Phase B+C+D done: 88 tests, 8+ roles fleshed out; Phase D mobile roles added: android_diagnose/android_security/ios_diagnose/ios_security + ios_security backend/linux_diagnose/macos_diagnose Python backends) | evidence: 12 roles (android/ios/linux/macos/windows diagnose+automation+kernel+security), 5 os_expert modules (logging_systems, os_events, package_management, security_architectures, system_buses), 6 connectors (adb, libimobiledevice, linux_namespaces, macos_security, windows_defender, windows_wmi). 8+ roles fleshed out with Python backends: android_diagnose+android_security+ios_diagnose (25 tests, commit 2465d8ca), ios_security+linux_diagnose+macos_diagnose backends (commit e06014d3). 88 tests pass; commits db2699da (9-feature wave), 816d7be6, 2465d8ca (3 mobile roles), e06014d3 (ios_security/linux/macos backends)
- [x] NF.7 — STS tokens: P1 AgentTokenModel + TokenMinter + TokenStore + P5 TokenReaper + cascade + daemon wiring | spec: docs/specs/FEATURE_STS_TOKENS.md | status: completed | evidence: P1-P5 done — minter+store+narrowing+reviver+revoker+hibernation wiring+audit+injector+TokenReaper+cascade+daemon wiring. 8 src files (sts/{minter,store,injector,narrowing,reviver,revoker,token_reaper,__init__}.py), 5 test files (sts/test_{minter,store,narrowing,reviver,revoker}.py), alembic migration 035, daemon hibernation wiring complete, P4 audit+injector tests, P5 TokenReaper + cascade + daemon wiring (24 tests, commit acdc1285). commits db2699da (9-feature wave P1-P3), 816d7be6 (P4 audit+injector), acdc1285 (P5 TokenReaper+cascade+daemon wiring)
...
- [-] NF.9 — Language expert collection: 8 roles + 5 knowledge modules | spec: docs/specs/FEATURE_LANGUAGE_EXPERT.md | status: in_progress (Phase A+B+C+D done: 282 tests, 8 roles+5 modules; Phase C 53 tests + Phase D 74 tests; molecule+integration tests added) | evidence: collection language with 8 roles (bom_detect, encoding_detect, font_analyze, homoglyph_scan, i18n_extract, locale_format, phonetic_transcribe, unicode_analyze), 5 knowledge modules (charset_map, homoglyph_data, locale_data, phonetic_data, unicode_data). Phase C (53 tests) + Phase D (74 tests, commit 773f9275) = 282 total tests pass. language molecule/playbooks/ + integration tests (test_integration_*.py); commits db2699da (9-feature wave), 816d7be6 (molecule+integration tests), 773f9275 (Phase D 74 tests)
- [x] NF.10 — enforce-stop.ts false-completion fix: comprehensive work-detection now checks CI+release+gate state | spec: N/A (bug fix) | status: completed | evidence: enforce-stop.ts work-detection extended beyond TASKS.md/ratchet.yml to also check CI status (ci-verdict), release completeness (verify-release-completeness), and gate status (gate-status); molecule made non-blocking in CI; false-completion incident documented in BUGS.md; commit 816d7be6
