...
- [x] S56.6 — Enforcement-plugin pressure-release deadlock fix: modified enforce-floor.ts, enforce-delegate.ts, enforce-multitask.ts, enforce-stop.ts + shared.ts to add failed-dispatch detection and degraded-mode pressure-release path; new tests test_multitask_pressure_release.py + test_enforce_stop_subagent_deficit.py | priority: high | effort: M | status: completed | evidence: BUGS.md 2026-07-27 entry + dirty-tree enforcement plugin changes + 2 new test files; fix uncommitted
