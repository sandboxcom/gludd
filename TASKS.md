...
- [x] MA-P4/P6 — Multitasking audit: close AGENTS.md message-shape loophole (clause (a) had no duration cap — an agent could send 100 consecutive zero-dispatch responses, each individually compliant) by capping consecutive zero-dispatch responses at 2; the 3rd MUST dispatch or explicitly justify; the 4th is a hard policy violation; cross-referenced the `_readStreak` counter in enforce-floor.ts (advisory >10 reads AND >60s, deny >20 reads AND >120s) and MAINTHREAD_THRESHOLD in enforce-delegate.ts; ALSO fixed P6 threshold documentation drift (AGENTS.md:9 documented enforce-delegate threshold as 1, actual MAINTHREAD_THRESHOLD=4 — corrected to 4, now consistent with the streak counter at AGENTS.md:1615 and the mainthread-budget rule at AGENTS.md:279) | evidence: AGENTS.md:1804 clause (a) cap text + read-streak cross-reference; AGENTS.md:9 threshold corrected 1→4
...
- [x] **Restart opencode** — operational meta-step completed; restart done. | evidence: commit 95d851fd
...
- [x] **B3.1.3 — Phase 2: subprocess extraction** — extract the daemon's writer path (event loop claim/review/reconcile) into a dedicated subprocess so the DB-write responsibility is isolated from the gunicorn HTTP workers. Builds on B3.1.1 (IPC broker) + B3.1.2 (read-only factory). **Slice 1-5 done.** | evidence: commit ed958fcf; slices 1-5 tests passed
...
- [x] **D-AB-5** — tool_loop sync: blocking tool_loop path synchronized against the event loop | evidence: commit b54e75ef
...
- [x] **D-#15** — accounting tenant scope: REFUTED. The system is single-tenant by design (SQLite-only, single-writer clamp per W3.5 decision; no multi-tenant query path exists). No per-tenant accounting scope is required; the finding does not apply. | evidence: commit b54e75ef
...
- [x] **D-AB-5/AB-8-residual** — regression tests for the tool_loop/engine asyncio.to_thread sync fixes landed. | evidence: tests/unit/test_ab5_ab8_regression.py 5 passed (TestAB5ToolLoopToThread + TestAB8EngineToThread); tests/unit/test_execution_engine_fixes.py 21 passed
...
- [x] **PR.1 — opencode skill `revealjs-presentation`** (codified ability) — new opencode skill at `.opencode/skills/revealjs-presentation/SKILL.md` documenting the workflow for generating/rebuilding a reveal.js deck from structured data (slides.yml + SVG diagrams + build script); skill frontmatter passes `make check-skills-frontmatter`. | evidence: commit 0f08af4b
- [x] **PR.2 — gludd ansible role `build_presentation`** (codified ability) — new Ansible role in `collections/ansible_collections/general_ludd/agent/roles/build_presentation/` that orchestrates deck regeneration (runs `build_deck.py` from the slides data source, emits HTML + bundled assets, fail-closed on missing diagrams); molecule scenario + role structure tests green. | evidence: commit 81bfea53
- [x] **PR.3 — SVG diagrams (architecture, event-loop, security-layers)** — three source SVG diagrams added under the presentation assets dir (`architecture.svg`, `event-loop.svg`, `security-layers.svg`); diagrams referenced by the deck and shipped as bundled reveal.js assets so the deck renders standalone. | evidence: commit 19dd629b
- [x] **PR.4 — Deck rewrite (non-technical audience, current stats, granular walkthrough)** — reveal.js deck rewritten for a non-technical audience: current project stats pulled from live data (not hardcoded), granular per-subsystem walkthrough slides (spine, event loop, security layers, observability, guardrails), narrative-driven slide ordering. | evidence: commit e2bd6e69
- [x] **PR.5 — build_deck.py fix (regenerates HTML from data)** — `build_deck.py` now regenerates the reveal.js HTML output from the structured slides data source on every run (was previously emitting stale HTML that drifted from the data); idempotent rebuild verified. | evidence: commit b4bd6c93
- [x] **PR.6 — pages.yml fix (builds before deploy)** — GitHub Pages workflow (`.github/workflows/pages.yml`) now runs the deck build step before the deploy step so the published site reflects the current slides data (was previously deploying stale HTML). | evidence: commit b4bd6c93
- [x] **PR.7 — README link fix** — broken presentation link in README.md corrected to point at the published Pages URL. | evidence: commit 0ce7fb38

## Phase Anti-Lying — Guardrails for verifiable claims (2026-07-09)

Three-layer anti-lying guardrail landing so false "done" claims are structurally impossible, not merely discouraged. Each row = one commit with its evidence. Mirrors the AGENTS.md "Verification Before Claim" section.

- [x] **AL-1** — `ae9861f3` guardrail(git): enforce-clean-tree plugin — `.opencode/plugin/enforce-clean-tree.ts` `tool.execute.before` hook DENIES task/agent/workflow dispatch when `git status --porcelain` is non-empty; prevents the recurring failure mode where a prior subagent's uncommitted changes cause pre-commit hook stash conflicts on the next push (forcing `-nv` bypasses that defeat the lint/secret guards). Fail-open; `GLUDD_CLEAN_TREE_ENFORCE=0` disables. | evidence: commit ae9861f3; tests/unit/test_clean_tree_plugin.py 27 passed (deny on dirty, allow on clean, fail-open)
...
- [x] **CGW-14** — failover gaps closed: structured all-down error, correlation-id propagation, failover_count facet, fallback concurrency cap; xfails flipped to plain assertions | evidence: commit 803b75c5; tests/e2e/test_failover_e2e.py 17 passed as plain tests; tests/unit/test_model_gateway.py 36 passed
- [x] **CGW-15** — DB perf indexes: migration 025 task_decisions(created_at) + todos(status,priority,created_at) composite | evidence: commit 97db7cc1; alembic parity suites 42 passed
- [x] **CGW-16** — spec review corrections applied: 70 work items with landed-verify annotations + MCP argv residual C27 | evidence: commit 2b17e7e3; docs/AGENTIC_IMPLEMENTATION_SPEC.md
...
- [x] **CGW-20** — security_backlog.py converted from an all-pass stub to a landed-guard static regression gate (`make security-backlog-gate`); D-07..D-30 IDs map to `docs/audit/NEW_FINDINGS_2026-06-16.md` (DISJOINT from TASKS.md Phase D's D-#N scheme — do not conflate). 3/24 items are real LANDED-VERIFIED probes (D-14 clone SSRF/RCE guard, D-18 audit-log wiring, D-27 RLIMIT sandbox caps); verified-OPEN residuals still needing a fix: D-07 (db/models.py CheckConstraint blob bounding), D-11 (no rate limiter on POST /api/todos creation), D-13 (no PRAGMA journal_size_limit on the WAL db), D-17 (no worker PSK rotation schedule). Report-only: scripts/backlog_audit.py references a nonexistent `make backlog-audit` target. | evidence: tests/unit/test_security_backlog.py 58 passed; make security-backlog-gate PASS (3 LANDED-VERIFIED / 21 OPEN)
- [x] **CGW-21** — remediation tick phase #52 (new _phase_remediate_blocked_tasks, PHASE_ORDER 16→17, RemediationActionRepository.exists_recent idempotency, dead daemon_state config wiring fixed) | evidence: tests/unit/test_remediation_phase_wiring.py + phase-pin updates 201 passed
- [x] **CGW-22** — pause persist-before-mutate + lock-free frozenset is_paused D7.1 | evidence: test_pause_persist_ordering.py + test_pause_concurrency.py, 689 passed pause sweep
- [x] **CGW-23** — hook-liveness harness (Node type-strip harness, 33 CI-skip checks converted, opt-in hook_live marker, SHA-pinned setup-node) | evidence: 43 passed 1 deselected + test_ci_workflow 59 passed
- [x] **CGW-24** — agent-liveness counting repair (activity-mtime tasks-dir ranking, per-session cache, window-filtered workflow glob, force-delegate streak reset) | evidence: test_agent_liveness.py 58 passed + test_force_delegate_hook.py 10 passed
- [x] **CGW-25** — file-claim TTL reaping (#53 registry side: crashed-worker claim expiry) | evidence: test_file_claims_ttl.py 25 passed; combined claim suites 77 passed
- [x] **CGW-26** — spend-limiter flush watermark API (SPD-1 limiter side: _seq + unflushed_records + mark_flushed + restore dedupe) | evidence: test_spend_limiter.py 47 passed; spend sweep 166 passed
- [x] **CGW-27** — tool-loop guard tests (5 untested branches: budget denial, per-iteration timeout, token cap, adversarial block, auditor gate) | evidence: test_tool_loop_guards.py 13 passed
- [x] **CGW-28** — payment CLI 4 defects fixed (list/show/add-readback error handling, ???? sentinel, docstring overstatement, empty-flag) + suite | evidence: test_cli_payment.py 47 passed
...
- [x] **text.complete tool-output pass-through fix** — enforce-multitask.ts + enforce-stop.ts text.complete hooks were blocking all tool output (Read/Grep/Glob results were replaced with enforcement messages). Fix: `isToolOutput` guard (`_input.role !== "assistant"`) returns early before enforcement. Additional gaps: enforce-multitask.ts tool.execute.before has no disengage escape; zeroStreak loads stale state from disk. 16 tests added (7 in test_multitask_plugin.py, 9 in test_plugin_behavior.py).
