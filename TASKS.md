...
| S56 | Session 56 — 2026-07-27 (Wave 3) | 0 | 6 | 100% |
| S56-W2 | Session 56 — 2026-07-27 (Wave 2) | 0 | 7 | 100% |
...
- [ ] SLURM-1 — Keep Slurm cost-cap lifecycle fixtures compliant with numeric-only scheduler job IDs so injection hardening remains enforced | paths: `tests/integration/test_bill_slurm_cost_cap_e2e.py`, `src/general_ludd/infra/slurm.py` | evidence: stepwise integration reproduced legacy `job-*` identifiers rejected by `_require_job_id`; numeric fixture alignment passes 8/8 cost-cap lifecycle tests, Ruff, 56,319/56,320 collection, and task integrity; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] COST-1 — Keep integration fixtures compliant with fail-closed nonzero pricing for enabled metered model profiles while preserving explicit unmetered/local zero-cost coverage | paths: `tests/integration/test_bill_compute_scheduling_e2e.py`, `tests/integration/test_budget_integrity.py`, `src/general_ludd/models/gateway.py`, `tests/unit/test_compute_aware_scheduling.py` | evidence: stepwise integrations reproduced stale zero-rate metered fixtures; scheduling alignment passes 45/45 tests and explicit-unmetered budget alignment passes 11/11, with Ruff, 56,319/56,320 collection, and task integrity green; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] GRC.1 — Make read-only Git repository evidence deterministic by deriving its RFC3339 evidence time from immutable repository state | paths: `src/general_ludd/git_release/topology.py`, `tests/unit/test_git_release_contracts.py`, `tests/integration/git_release/test_assess_to_plan.py` | evidence: red-first unit and integration regressions reproduced wall-clock evidence drift; immutable-HEAD timestamp implementation passes 38/38 focused unit/integration tests, Ruff, 56,319/56,320 collection, task integrity, and ledger validation; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] LOG.1 — Keep the log-analyzer role isolated to caller-selected inputs and outputs while preserving task/failure events across ansible-core callback API changes | paths: `collections/ansible_collections/general_ludd/operations/roles/log_analyzer/tasks/main.yml`, `src/general_ludd/ansible/core_runner.py`, `tests/integration/test_log_analyzer.py`, `docs/LOG_ANALYZER_EXAMPLE.md` | evidence: stepwise integration reproduced role exit 2 with no callback events; isolated paths and callback initialization pass 19/19 focused role tests, 60/60 delegated runner regressions, and 47/47 independent core/event/env/timeout regressions; Ruff, 56,319/56,320 collection, task integrity, and ledger validation pass; Ansible 2.19 community compatibility evidence documented; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] MEM.1 — Apply explicit TEMPR date ranges as one fail-closed eligibility boundary across semantic, BM25, temporal, graph, and RRF results | paths: `src/general_ludd/memory/tempr_retriever.py`, `tests/unit/test_tempr_retriever.py`, `tests/integration/test_memory_pipeline_e2e.py`, `docs/features/TEMPR_DATE_RANGE_FILTERING.md` | evidence: stepwise integration reproduced a 30-day-old semantic/BM25 result bypassing a 3-day range; inclusive UTC-normalized prefiltering and malformed/missing timestamp exclusion pass 60/60 focused unit/integration tests and Ruff; retrieval-community operational evidence documented; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] AZL.1 — Replace the Azure Container Apps echo-server stand-in with a deploy-local, provider-validated, scale-to-zero GPU vLLM stack; prove live model inference and verified teardown | paths: `infra/terraform/stacks/azure-container-app-vllm/main.tf`, `infra/terraform/stacks/azure-container-app-vllm/outputs.tf`, `infra/terraform/stacks/azure-container-app-vllm/variables.tf`, `infra/terraform/modules/azure-container-app-vllm/main.tf`, `infra/terraform/modules/azure-container-app-vllm/variables.tf`, `infra/terraform/modules/azure-container-app-vllm/outputs.tf`, `tests/integration/test_bill4_terraform_watchdog_wiring.py`, `tests/integration/test_bill5_spot_preemptible_wiring.py`, `tests/e2e/providers/test_azure_provision_e2e.py`, `docs/research/AZURE_TERRAFORM_EVENT_ELASTICITY_E2E_EVIDENCE.md` | evidence: 2026-08-01 live Azure apply, scale-from-zero `/v1/models` and chat inference, streamed Terraform events, app shutdown, environment/resource-group destroy, and independent `leaked_resources=0` verification passed; Azure Container Apps stack now has bounded-cost/TTL watchdog parity (10/10 integration) and a false-only unsupported-spot contract (17/17 integration), with all 18 provider-version contracts, 56,319/56,320 collection, and task integrity green; full repository gate pending | priority: high | effort: L | status: in_progress
- [ ] AZL.2 — Replace static work-item cost tiers with exact Azure Retail Prices meter selection and delayed billed-cost reconciliation | paths: `src/general_ludd/infra/azure_retail_pricing.py`, `src/general_ludd/infra/azure_cost_reconciliation.py`, `src/general_ludd/infra/deploy_strategy.py`, `tests/unit/test_azure_retail_pricing.py`, `tests/unit/test_azure_cost_reconciliation.py`, `tests/unit/test_deploy_strategy.py`, `tests/e2e/providers/test_azure_retail_pricing_e2e.py`, `tests/e2e/providers/test_azure_provision_e2e.py`, `pyproject.toml`, `uv.lock`, `docs/research/AZURE_TERRAFORM_EVENT_ELASTICITY_E2E_EVIDENCE.md` | evidence: exact pre-deploy Container Apps GPU/vCPU/memory meters and credential-free live East US contract passed; red-first reconciliation foundation now preserves immutable ARM/meter/work identity, uses the official Azure Cost Management Query client with exact resource scoping and schema-drift rejection, fails closed after the 72-hour data window, attributes ancillary rows, and computes homogeneous 20-run MAPE/p95/bias/ceiling metrics (28 focused tests green); durable ActualCost Export ingestion, fenced persistence/finality, exact VM warmup, complete ancillary forecasting, and real calibrated cohort evidence remain pending | priority: high | effort: L | status: in_progress
- [ ] MWK.1 — Implement durable PostgreSQL event/work transport with fenced cross-Gunicorn-worker claims and immediate readiness consumption | evidence: 2026-08-01 namespaced live target applied 37 PostgreSQL migrations, proved disjoint claims from two processes, read a Terraform event from a second connection, booted two real Gunicorn workers without optional-service races, and removed/stopped all test resources; full repository gate pending | priority: high | effort: L | status: in_progress
- [ ] FPX.1 — Prove every FPS fixture's menu, controls, gameplay transition, captured video, and pinned online-reference comparison on available Gludd compute | evidence: prompt/reference unit slices green; 2026-08-01 live Azure endpoint reached readiness and streamed teardown, exposing the unauthenticated-vLLM client-key boundary now covered by `test_gateway_factory_supplies_placeholder_key_for_unauthenticated_vllm`; paid-run resource preflight now measures the workspace Data volume, defers cleanup during project validation, preserves shared pytest roots, and boundedly removes only namespaced stopped Podman VMs; per-game live compute rerun pending | priority: high | effort: L | status: in_progress
- [ ] TUI.1 — Make every Rich TUI table obey its measured panel width and preserve safe defaults for non-interactive callers | paths: `src/general_ludd/cli.py`, `src/general_ludd/tui/runner.py`, `src/general_ludd/tui/tables.py`, `tests/unit/test_tui_screen_real_estate.py`, `docs/features/TUI_RESPONSIVE_TABLES.md` | evidence: red-first screen-real-estate slice had 25 failures; implementation reduced that to 40/40 passing across 40–200 columns, surrounding TUI suite 318/318 passed, shared table factory coverage 90%, scoped Ruff and mypy green; full repository gate pending | priority: medium | effort: S | status: in_progress
- [ ] ENF.1 — Keep enforcement plugin imports process-pure and loader-safe so runtime checks cannot multiply detached gate trees or crash OpenCode discovery | paths: `.opencode/lib/multitask_config.ts`, `.opencode/plugin/enforce-multitask.ts`, `.opencode/plugin/enforce-multitask.test.node.mjs`, `.opencode/plugin/impl/enforce_stop_impl.ts`, `scripts/test_hook_runtime.py`, `tests/unit/test_enforcement_gate_spawn_safety.py`, `tests/unit/test_multitasking_grinding_block.py`, `docs/features/ENFORCEMENT_PROCESS_ISOLATION.md` | evidence: red-first process tests exposed both detached gate launchers and the full gate later exposed five loader-visible named exports; plugin-owned child creation is removed and validated multitask configuration now lives outside the auto-discovered directory; loader suite 115/115, dedicated Node behavior 34/34, enforcement runtime 124/124, Node compatibility 5/5, plugin manifest 99/99, and 13/13 enforcement verification pass; post-verification process check still required after the full repository gate | priority: high | effort: S | status: in_progress
- [ ] ENF.3 — Keep enforcement project-root discovery portable and isolated so markerless or invalidly configured sessions never borrow another developer checkout | paths: `.opencode/lib/shared.ts`, `tests/e2e/test_enforce_stop_live.py`, `docs/features/ENFORCEMENT_PROCESS_ISOLATION.md`, `docs/ENFORCEMENT_ARCHITECTURE.md`, `docs/CONFIG_REFERENCE.md` | evidence: red-first clean-temp live hook reproduced an unrelated `/Users/shawnwilson/gludd` ledger block; missing/invalid override matrix failed 2/5 before the fix, then project-root matrix passed 7/7, the full live stop suite passed 25/25, hook runtime passed 124/124, multitask Node behavior passed 34/34, and Node parse/factory compatibility passed 5/5; full repository gate pending | priority: high | effort: S | status: in_progress
- [x] ENF.2 — Serialize every hardcoded Gludd enforcement-state test through one canonical xdist group and remove optional-file snapshot TOCTOU races | paths: `tests/conftest.py`, `tests/unit/_hook_fixtures.py`, `tests/unit/test_enforcement_shared_state_isolation.py`, `tests/unit/test_enforce_stop_mixed_response.py`, `tests/unit/test_enforce_stop_runtime.py`, `tests/unit/test_enforce_stop_subagent_deficit.py`, `tests/unit/test_force_delegate_hook.py`, `tests/unit/test_enforcement_reload_targets.py`, `tests/e2e/test_deadline_e2e.py`, `tests/e2e/test_enforcement_plugin_e2e.py`, `tests/e2e/test_enforcement_e2e.py`, `tests/e2e/test_enforce_stop_e2e_block.py`, `docs/features/ENFORCEMENT_PROCESS_ISOLATION.md` | evidence: red-first isolation slice failed 9/11 on the missing canonical grouping and race-safe snapshot contracts; source/fixture grouping, five legacy aliases, unrelated port/hot-reload preservation, and disappearance-during-read now pass 20/20; affected live enforcement suites pass 227/227 under xdist with no Azure or network use; scoped Ruff, 220-spec lint, task ledger/integrity, and collection of 56,219 tests are green | priority: high | effort: S | status: completed
- [x] MTG.1 — Preserve cheap read/grep/glob investigation while prioritizing specific mutation-grinding and zero-dispatch diagnostics over the configured-minimum fallback | paths: `.opencode/plugin/enforce-multitask.ts`, `.opencode/lib/multitask_config.ts`, `tests/unit/test_multitask_plugin.py`, `tests/unit/test_multitask_min_dispatch.py`, `tests/unit/test_multitasking_grinding_block.py`, `tests/e2e/test_behavior_enforcement.py`, `tests/e2e/test_multitask_e2e.py`, `docs/features/ENFORCEMENT_PROCESS_ISOLATION.md` | evidence: both existing behavior regressions failed red with `CONFIGURED MINIMUM BLOCK`; each Node invocation now has isolated multitask state and hot-module paths, the legacy grinding case uses five same-message mutations, zero-dispatch evaluation precedes the fallback, all 250 broad legacy multitask contracts pass under the adaptive opt-in policy, focused behavior/read-exclusion passes 10/10, enforcement runtime passes 124/124, Node v26 passes 5/5, and enforcement verification passes 13/13 | priority: high | effort: S | status: completed
- [ ] WMT.1 — Keep worker model generation off the event loop while making deployment and resource-lifecycle signal installation safe for non-main-thread lazy imports | paths: `src/general_ludd/infra/deployment.py`, `src/general_ludd/cloud/resource_lifecycle.py`, `tests/unit/test_worker_model_thread_signal.py`, `docs/features/WORKER_MODEL_THREAD_SIGNAL_SAFETY.md` | evidence: red-first lifecycle regression reproduced `ValueError: signal only works in main thread`; focused worker-thread regressions are 2/2 green, the original generation E2E is 1/1 green, and the port-8000 nested worker wrapper is 1/1 green; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] ATW.1 — Make bounded CoreAnsibleRunner workers safe in multithreaded Gunicorn/Python 3.14 processes without weakening seccomp, deadline, process-group, or registry guarantees | paths: `src/general_ludd/ansible/core_runner.py`, `tests/unit/test_ansible_timeout_process_safety.py`, `tests/security/test_ansible_redteam.py`, `docs/security/ansible-timeout-process-safety.md`, `docs/security/index.md` | evidence: warning-as-error red-first regression became green; focused timeout/security slice 26/26 and broader Ansible worker slice 137/137 passed with no `popen_fork.py` warning; scoped Ruff and mypy green; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] TDQ.1 — Preserve the daemon's plain startup todo state while guaranteeing the degraded-mode fallback becomes a bounded FIFO queue on first access | paths: `src/general_ludd/daemon.py`, `tests/e2e/test_cli_daemon_lifecycle_workflows.py`, `tests/unit/test_todos_router_bounds.py`, `docs/features/BOUNDED_DEGRADED_TODO_QUEUE.md` | evidence: fail-fast full-suite run exposed the unbounded pre-created deque after 918 passes; focused red run reproduced three contradictory lifecycle/bounds failures, then the original failure and full bounded-queue slice passed 8/8; scoped Ruff and mypy green; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] RDY.1 — Keep pre-lifespan readiness tests isolated from TestClient startup so `daemon_not_initialized` cannot be masked by real database and event-loop initialization | paths: `tests/e2e/test_cli_daemon_lifecycle_workflows.py`, `docs/features/READINESS_LIFESPAN_TESTING.md` | evidence: fail-fast full-suite run exposed the contradiction after 927 passes; focused readiness matrix reproduced 1 failure and 33 passes while the context-managed test also emitted unrelated persistent-database schema errors; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] ARP.1 — Keep public-route auth tests aligned with the live TestClient lifespan and isolate every implicit daemon database from persistent user state | paths: `tests/e2e/test_cli_daemon_lifecycle_workflows.py`, `docs/features/READINESS_LIFESPAN_TESTING.md` | evidence: stepwise full-suite run exposed `200 ready` versus an invalid context-managed `503` expectation and persistent stale-schema errors; red-first focused reproduction failed 1/1; corrected node passes 1/1 and the complete isolated daemon lifecycle module passes 62/62 without stale-schema errors; scoped Ruff, 220-spec lint, task integrity, and ledger validation pass; 56,219 tests collect with 0 errors; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] STA.1 — Preserve `gludd test self` as canonical while restoring the top-level `gludd selftest` compatibility alias with identical handler, daemon URL, and help semantics | paths: `src/general_ludd/cli.py`, `tests/e2e/test_cli_full_coverage.py`, `tests/unit/test_cli_subcommands.py`, `tests/unit/test_selftest.py`, `docs/features/CLI_SELFTEST_COMPATIBILITY.md` | evidence: red-first focused CLI slice reproduced 4 failures and 78 passes because the removed alias exited with parser error 2 before dispatch; shared public-API parser configuration now keeps both spellings aligned and the focused CLI slice passes 85/85; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] SEC.1 — Implement the complete configurable security and sandbox hardening specification, close every open D-08–D-30 control, and reduce SAST to zero high/medium with every low fixed or time-bounded by tested adjudication | paths: `docs/specs/FEATURE_SECURITY_SANDBOX_HARDENING.md`, `docs/security/audit-observability.md`, `scripts/summarize_sast.py`, `src/general_ludd/ansible/unsafe.py`, `src/general_ludd/ansible/runner.py`, `src/general_ludd/ansible/templating.py`, `src/general_ludd/mcp/transport.py`, `src/general_ludd/security/security_backlog.py`, `src/general_ludd/security/state.py`, `tests/unit/test_ansible_extravars_validation.py`, `tests/unit/test_mcp_transport.py`, `tests/unit/test_mcp_transport_stderr.py`, `tests/unit/test_security_audit_observability.py`, `tests/unit/test_security_backlog.py`, `tests/unit/test_security_sandbox_hardening_spec.py`, `tests/unit/test_security_state.py` | evidence: bounded audit completed in 54.798s with heartbeats and clean secrets/dependencies; D-08 and D-24 are regression-probed, leaving 10 landed/14 open controls; independent post-migration SAST is 0 high/0 medium/505 low with B104/B108/B310/B323/B608 all zero and no suppressions; focused secure-state verification is 133/133, Ruff and scoped mypy green; every remaining low and the 14 open controls still require remediation or tested time-bounded adjudication; full repository gate pending | priority: high | effort: XL | status: in_progress
- [ ] SEC.2 — Eliminate non-temporary-path medium SAST findings with substantive configurable network, SQL, SSRF, TLS and onboarding-input controls | paths: `src/general_ludd/config/user_config.py`, `src/general_ludd/daemon.py`, `src/general_ludd/history/git_indexer.py`, `src/general_ludd/infra/azure_retail_pricing.py`, `src/general_ludd/infra/discovery.py`, `src/general_ludd/infra/terraform.py`, `src/general_ludd/networking/scapy_adapter.py`, `src/general_ludd/onboard/azure.py`, `src/general_ludd/onboard/gcp.py`, `tests/unit/test_onboard_init.py`, `tests/unit/test_sast_medium_network_sql.py` | evidence: red-first 14-test slice became 15/15 green; affected neighborhood 364/364 passed; Ruff and scoped mypy green; Bandit dropped from 46 to 34 medium findings with B104/B310/B323/B608 all zero and no live cloud/network calls; full repository gate pending | priority: high | effort: M | status: in_progress
- [ ] SEC.3 — Replace all predictable/global runtime-state paths with one configurable, private, project-namespaced allocator and preserve safe legacy plugin compatibility | paths: `src/general_ludd/security/state.py`, `src/general_ludd/security/sandboxes/state.py`, `src/general_ludd/ag2_lifecycle/hooks.py`, `src/general_ludd/execution/engine.py`, `src/general_ludd/git_automation/repo.py`, `src/general_ludd/routers/integrity.py`, `src/general_ludd/self_update/grinding_detector.py`, `tests/unit/test_security_state.py`, `tests/unit/test_b108_secure_state_migration.py` | evidence: all 34 B108 call sites migrated through configurable `GLUDD_STATE_DIR`; 0700 namespaces, 0600 durable writes, owner/symlink/containment checks and exact cleanup are regression-tested; impacted compatibility suite 510/510, parent verification 133/133, Ruff and scoped mypy green; independent SAST is 0 high/0 medium with B108=0 and no suppression; full repository gate pending | priority: high | effort: M | status: in_progress
- [ ] SEC.4 — Bind every durably sealed sandbox attestation to the exact evaluated draft so a store cannot substitute a different integrity-valid payload | paths: `src/general_ludd/security/sandboxes/dispatch.py`, `tests/unit/test_sandbox_dispatch_boundary.py`, `tests/unit/test_sandbox_policy_profiles.py`, `tests/unit/test_sandbox_runtime_attestation.py`, `tests/unit/test_sandbox_state.py`, `tests/unit/test_secure_xml.py`, `tests/unit/test_url_fetch_security.py` | evidence: red store-swap regression initially failed to raise and now passes; parent focused run is 137/137 with per-file coverage 91–100% and 95.11% aggregate across six security/sandbox modules; Ruff and scoped source mypy green; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] COV.1 — Make the coverage-gap gate resolve actual test imports and require behavioral tests for every genuine new module instead of relying on filename guesses or allowlist growth | paths: `scripts/check_coverage_gaps.py`, `tests/unit/test_check_coverage_gaps.py`, `tests/unit/test_materials_textiles.py`, `tests/unit/test_routers_experts.py`, `tests/unit/test_self_improve_evaluator.py` | evidence: four red mapper regressions exposed direct-import, re-export, alias and file-loader misses; repository audit improved from 298/809 recognized modules and 70 new gaps to 802/809 recognized and 0 new gaps without baseline changes; 81/81 focused tests pass; genuine modules measure 98%, 100% and 99% with 98.75% aggregate; Ruff and scoped mypy green; full repository gate pending | priority: high | effort: M | status: in_progress
- [ ] IAM.1 — Execute cross-provider IAM generation as maintainable offline code and keep the Azure action catalog consistent with verified policy inputs | paths: `Makefile`, `src/general_ludd/cloud/validate_all.py`, `src/general_ludd/azure/rbac_validator.py`, `tests/unit/test_cloud_iam_expert.py`, `tests/unit/test_azure_rbac_validator.py` | evidence: red-first gate reproduced an invalid compound Python statement and a missing verified Container Apps join action; 168/168 focused IAM tests pass; `make validate-all-cloud-iam` generates and validates Azure/AWS/GCP monitor roles with 0 Azure errors and 0 warnings; Ruff, scoped mypy, target contract, and duplicate-target checks pass; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] GT.1 — Keep unpublished-development validation offline while retaining tag-dependent artifact, provenance, and changelog checks as explicit release targets | paths: `Makefile`, `tests/unit/test_gate_release_boundary.py` | evidence: full gate failed red because its prerequisite invoked AC010 without a release tag; red-first regression proved AC010/AC011/AC015 were coupled to the local gate; 2/2 boundary tests now pass and all three live checks remain explicit release targets; target-contract and duplicate-target checks pass; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] DEP.1 — Enforce reproducible dependency resolution through uv's exact lock without replacing compatible package metadata with environment-specific pins | paths: `Makefile`, `scripts/check_dependency_pinning.py`, `tests/unit/test_release_pipeline_checks.py`, `docs/specs/BEHAVIORAL_SPECS.md` | evidence: full gate red-flagged every compatible range despite 331 exact locked packages; four red-first regressions proved satisfying ranges, incompatible versions, and missing entries; 12/12 focused tests pass; `uv lock --check` and AC012 now pass for 331 packages; Ruff, scoped mypy, target-contract and duplicate-target checks pass; forum findings and security rationale documented; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] VER.1 — Preserve complete PEP 440 prerelease versions across every file in the atomic version gate | paths: `scripts/check_version_bump_atomicity.py`, `tests/unit/test_release_pipeline_checks.py` | evidence: full gate red-reported README and CHANGELOG as `0.1.0-beta` while both contain `0.1.0-beta.3`; red-first repository-style regression reproduced truncation; 19/19 focused version tests pass and AC016 now reports all files at `0.1.0-beta.3`; scoped mypy green; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] PLG.1 — Restore the documented enforcement hook order so audit and stale-context guards run before downstream dispatch/release/task hooks | paths: `opencode.json`, `tests/e2e/test_enforcement_e2e.py`, `docs/ENFORCEMENT_PLUGIN_REGISTRY.md` | evidence: fail-fast full-suite run exposed the configuration order mismatch after 107 passing tests; existing red contract and registry both require audit/context at positions 23/24; focused E2E order regression passes, manifest is 99/99, and OpenCode integrity passes; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] PLG.2 — Preserve observable source-level enforcement contracts after shared configuration and lean-entrypoint extraction | paths: `.opencode/plugin/enforce-multitask.ts`, `.opencode/plugin/enforce-stop.ts`, `.opencode/lib/multitask_config.ts`, `.opencode/plugin/impl/enforce_stop_impl.ts`, `tests/e2e/test_enforcement_plugin_e2e.py` | evidence: fail-fast full-suite run exposed the missing multitask state contract after 166 passing tests; the full source-contract class then exposed the extracted delegate threshold; existing red contracts now pass; full repository gate pending | priority: high | effort: S | status: in_progress
...
- [ ] APR.1 — Separate `audit-plugins --project` logical names from explicit paths while preserving strict sandbox-state validation and fail-closed traversal/symlink handling | paths: `src/general_ludd/cli_audit_plugins.py`, `tests/unit/test_cli_audit_plugins.py`, `tests/e2e/test_cli_workflow.py`, `docs/features/AUDIT_PLUGIN_PROJECT_RESOLUTION.md` | evidence: red-first audit CLI slice reproduced 7 failures; logical-name, explicit-path, missing/file/traversal/symlink, artifact-state, and E2E propagation matrix passes 24/24; full repository gate pending | priority: high | effort: S | status: in_progress
- [ ] MPL.1 — D-30 phase-one buffered model gateway limits: configurable request/response byte and token ceilings plus raw tool-call cap, typed payload-free rejection, cache revalidation, and side-effect-free oversize handling | paths: `src/general_ludd/models/gateway.py`, `tests/unit/test_model_gateway_payload_limits.py`, `tests/unit/test_model_gateway_fallback.py`, `docs/features/MODEL_GATEWAY_PAYLOAD_LIMITS.md` | evidence: red-first focused test failed collection on the missing typed error; 23 focused payload-limit tests and a 297-test selected gateway regression set pass; `gateway.py` branch-inclusive coverage is 85.03%; exact UTF-8, trusted-counter fallback, consistent provider metadata, cache/provider no-bill/no-trace/no-meter/no-persist paths, fallback propagation, guarded raw-model construction, and broadcaster-error redaction are covered; full repository gate pending | priority: high | effort: M | status: in_progress
...
## Session 56 Active — 2026-07-27

- [x] S56.1 — Behavioral enforcement guards (AB001-AB060): 60 behavioral specs with enforcement mechanisms covering pre-dispatch checklist, spec quality, CI discipline, task-ledger integrity, worktree discipline, commit-message format, merge safety, plugin exports, disk discipline, session recovery, spec effectiveness | priority: high | effort: M | status: completed | evidence: commits 0d080ca5 (AB020-AB060), a40eaadb (AB001-AB019), e5300c57 (4 guards); 60 specs defined in BEHAVIORAL_SPECS.md, tracked in test_behavioral_enforcement.py; 29 audit/enforcement scripts in scripts/; lint 0
- [x] S56.2 — Governance P3-P6 data expansion: tax/currency (23 countries), conflicts (10 wars/15 treaties), civic services (26), decision_makers (50+ officials), military (25 countries), licenses (15 types) | priority: medium | effort: M | status: completed | evidence: commit 0d080ca5; 759 governance tests pass total
- [x] S56.3 — Security hardening: bandit.yaml config + post-commit secrets test | priority: high | effort: S | status: completed | evidence: commit 0d080ca5
- [x] S56.4 — CI diagnostic: run 30310764100 on HEAD 0d080ca5, conclusion=failure (RED) — root cause: pkg_resources (setuptools) import crash in Python 3.14 binary smoke test | priority: high | effort: S | status: completed | evidence: `make ci-verdict BRANCH=development` → conclusion=failure, run 30310764100
- [x] S56.5 — CI fix: pkg_resources force-assignment stub + store.py importlib pattern for fs imports to avoid E402 lint | priority: high | effort: M | status: completed | evidence: commit 19f937e8; fresh CI triggered (run 30312772822, in_progress)
- [x] S56.6 — Enforcement-plugin pressure-release deadlock fix: modified enforce-floor.ts, enforce-delegate.ts, enforce-multitask.ts, enforce-stop.ts + shared.ts to add failed-dispatch detection and degraded-mode pressure-release path; new tests test_multitask_pressure_release.py + test_enforce_stop_subagent_deficit.py; 30 Makefile targets, 30 scripts, 140+ structural tests | priority: high | effort: M | status: completed | evidence: commit 63d971bf (beta.3 wave 4); BUGS.md 2026-07-27 entry resolved; 2 new test files; gate-lite behavioral integration
- [x] S56.7 — Enforcement-plugin-no-pressure-release bug documented in BUGS.md: agent deadlocks when dispatches fail while streak counter blocks inline recovery reads/edits | priority: high | effort: S | status: completed | evidence: BUGS.md 2026-07-27 entry (updated with fix details)
- [x] S56.8 — SESSION.md + TASKS.md updated for Session 56 HEAD 19f937e8, CI PENDING run 30312772822, pressure-release fix, CI pkg_resources fix, 60 behavioral specs AB001-AB060 complete | priority: medium | effort: S | status: completed | evidence: commits pending
...
- [x] S58.1 — Materials Engineering: contracts, units, selection, strength, polymers, metals, joining, machining, additive, textiles, tolerance, failure, process_planning, source_registry, property_store, simulation | evidence: 709 tests pass, commit dd2aec1a+ | priority: high | effort: L | status: completed
- [x] S58.2 — Chemistry Expert: schemas, entities, reactions, stoichiometry, safety, properties, protocols, inventory, cheminformatics, thermo_kinetics, spectroscopy, analytical, validation, compute, promotion, provenance, electrochemistry, process, api, router, policy | evidence: 709 tests pass | priority: high | effort: L | status: completed
- [x] S58.3 — AI/ML Expert: router, schemas, evidence, registries, reasoning, retrieval, adaptation, evaluation, datasets, research, distillation, policy, speech, vision, images, accelerators, promotion, world_models, simulators | evidence: 709 tests pass | priority: high | effort: L | status: completed
- [x] S58.4 — Git Release Captain: contracts, topology, evidence, helper_catalog, helper_ranker, release_state, deployment, provenance, source_registry + 14 collection roles | evidence: 709 tests pass | priority: high | effort: L | status: completed
- [x] S58.5 — 52 collection roles scaffolded (11 materials + 15 chemistry + 15 ai_ml + 11 git_release) | evidence: make scaffold-collection-roles | priority: medium | effort: M | status: completed
- [x] S58.6 — OSS tools survey completed | evidence: docs/research/OSS_TOOLS_SURVEY.md | priority: low | effort: S | status: completed
...
- [x] T-BETA3-COVERAGE — Instrumented E2E branch coverage: repair environment, prove >=85% aggregate and >=75% per file | evidence: 14-shard beta.3 fan-in reported 91% aggregate with 0/787 files below 75%; exact release run `30540885831` completed coverage successfully at `f11486903b258a714c359e739fce90ec343f108f` | priority: medium | effort: M | status: completed
...
- [x] T-BETA3-HOOKS — Hook-runtime regressions: fix permission/status-summary blocking failures and rerun runtime harness | evidence: exact-master run `30538937151` and exact-tag run `30540885831` completed both Python gate jobs and all test shards successfully at `f11486903b258a714c359e739fce90ec343f108f` | priority: medium | effort: M | status: completed
- [x] T-BETA3-RELEASE — Beta.3 release readiness: complete remaining release/CI Git automation operations, gate, exact-SHA CI, and release validation | evidence: immutable tag `v0.1.0-beta.3` at `f11486903b258a714c359e739fce90ec343f108f`; hosted run `30540885831` success; `make verify-release-artifact TAG=v0.1.0-beta.3` and `make verify-release-completeness TAG=v0.1.0-beta.3` PASS; 21 non-empty GitHub release assets | priority: medium | effort: M | status: completed
...
- [x] T-RESOURCE-DISK-PRUNE — Replace silent unbounded uv cache deletion with upstream-recommended bounded, visible pruning | paths: `scripts/disk-guard.sh`, `tests/unit/test_process_monitoring.py` | priority: high | effort: S | status: completed | evidence: regression test failed before implementation and passed after; bounded behavioral run emitted one-second `UV_CACHE_PRUNE_HEARTBEAT` progress, stopped the lock wait after five seconds, reported rc=2, and continued the remaining cleanup phases; full process-monitoring file reached 35 passed with two unrelated pre-existing disengage-audit failures identified for immediate repair
- [x] T-ENFORCEMENT-DISENGAGE-AUDIT — Restore the fail-open JSONL audit record, visible cumulative count, and single-operation disengagement contract | paths: `Makefile`, `tests/unit/test_disengage_audit.py`, `tests/unit/test_granular_disengage.py`, `tests/unit/test_process_monitoring.py` | priority: high | effort: S | status: completed | evidence: first focused run exposed five failures (missing literal append contract and missing `disengage-next`); implementation rerun passed 60/60; make-target contract passed for 17 targets; duplicate-target guard passed for 861 targets; behavioral `make disengage-enforcement` appended a record and displayed its cumulative count
- [x] T-AZURE-GAME-TYPECHECK — Restore strict typing for phased deployment result shapes and typed FPS frame arrays after the Azure/game feature merges | paths: `src/general_ludd/infra/deploy_strategy.py`, `src/general_ludd/cloud/game_gen.py`, `tests/unit/test_deploy_strategy.py`, `tests/unit/test_game_gen.py` | priority: high | effort: S | status: completed | evidence: full strict mypy passed across 919 source files; focused deployment/game unit suite passed 35 with one pygame-dependent skip; scoped lint passed
- [x] T-AZURE-DAEMON-EVENTBUS — Wire the production daemon DeploymentManager to the daemon-owned EventBus so Terraform lifecycle events enter live history and subscribers immediately | paths: `src/general_ludd/daemon.py`, `tests/unit/test_daemon_startup.py` | priority: high | effort: S | status: completed | evidence: focused regression failed with manager bus `None` before implementation and passed after; daemon-startup plus deployment-telemetry suite passed 9/9; strict mypy passed all 919 source files; scoped lint passed
...
- [x] T-AZURE-LIVE-ENV-PATH — Parameterize and fail-close the sourced Azure E2E credential-file path with a non-provisioning behavioral contract | paths: `Makefile`, `config/make_target_contract.json`, `tests/unit/test_e2e_runner_target.py`, `tests/fixtures/azure-e2e.env.example`, `docs/azure-iam-setup.md` | evidence: TDD regression failed before implementation then passed; target/contract suite 21 passed; make-target contract PASS (17 targets); documented fixture behavior and `/tmp/general-ludd.env` validate-only behavior both PASS; 55,538 tests collected with zero collection errors | priority: high | effort: S | status: completed
- [x] T-AZURE-CLEANUP-VERIFY — Fail closed on interrupted live Azure runs with credential-safe, bounded, visible deletion and authoritative resource-group absence polling | paths: `Makefile`, `config/make_target_contract.json`, `tests/unit/test_e2e_runner_target.py`, `docs/azure-iam-setup.md`, `docs/research/AZURE_TERRAFORM_EVENT_ELASTICITY_E2E_EVIDENCE.md` | priority: high | effort: S | status: completed | evidence: red-first cleanup contract failed before implementation; focused runner/contract suite passed 24/24 including deletion-free success and simulated timeout; make-target contract passed for 18 targets; duplicate-target guard passed for 861 targets; documented behavioral example emitted `CLEANUP_VERIFIED leaked_resources=0` without Azure access
- [x] T-AZURE-LIVE-RESEARCH — Convert long-lived Azure/Terraform/Gunicorn operator failures into an executable live-E2E contract and select a mature durable cross-worker transport | paths: `docs/research/AZURE_TERRAFORM_EVENT_ELASTICITY_E2E_EVIDENCE.md`, `docs/research/GUNICORN_CROSS_WORKER_TRANSPORT_DECISION.md` | evidence: commits `4844396e`, `05ecf5fa`; primary docs plus linked user reports; lint and 55,537-test collection validation passed | priority: high | effort: M | status: completed
...
<!-- T-BETA3-OPENCode additional path registration: .opencode/lib/hot_reload.ts scripts/build_hot_modules.js tests/unit/test_hot_reload_module.py tests/unit/test_hot_reload_proxy.py tests/e2e/test_opencode_plugin_load.py tests/e2e/test_opencode_boot_e2e.py scripts/opencode_boot_smoke.py -->
...
- [x] T-BETA3-AUDIT-OBSERVABILITY — AB061-AB080 observability enforcement audit script verifying enforcement mechanisms per spec | paths: scripts/audit_observability.py | priority: medium | effort: M | status: completed | evidence: script committed, gate prereq check passes
- [x] T-BETA3-RELEASE-CUT-CHECKLIST — Pre-flight release-cut checklist verifying all pre-release conditions before tag push | paths: scripts/release_cut_checklist.py | priority: high | effort: M | status: completed | evidence: script committed, gate prereq check passes
