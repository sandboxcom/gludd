...
- [ ] COST.1 — Keep compute-scheduling integration fixtures compliant with fail-closed nonzero pricing for enabled metered model profiles | paths: `tests/integration/test_bill_compute_scheduling_e2e.py`, `src/general_ludd/models/gateway.py`, `tests/unit/test_compute_aware_scheduling.py` | evidence: stepwise integration reproduced stale zero-rate metered fixtures; canonical nonzero-rate alignment passes 45/45 integration/unit scheduling tests, Ruff, 56,319/56,320 collection, and task integrity; full repository gate pending | priority: high | effort: S | status: in_progress
