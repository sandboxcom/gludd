...
- [ ] SEC.1 — Implement the complete configurable security and sandbox hardening specification, close every open D-08–D-30 control, and reduce SAST to zero high/medium with every low fixed or time-bounded by tested adjudication | paths: `docs/specs/FEATURE_SECURITY_SANDBOX_HARDENING.md`, `tests/unit/test_security_sandbox_hardening_spec.py` | evidence: live bounded `make security-audit` completed in 54.798s with secrets/dependency phases green and continuous heartbeats; current honest inventory is 16 open backlog controls, 0 high/46 medium/506 low Bandit findings, and no SAST baseline; implementation and full repository gate pending | priority: high | effort: XL | status: in_progress
