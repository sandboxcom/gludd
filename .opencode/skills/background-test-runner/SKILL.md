...
```
make test-bg-status TESTFILE='tests/unit/test_foo.py'
```
...
```
make test-bg-kill TESTFILE='tests/unit/test_foo.py'
```
...
3. **Poll from subagents, not the main thread** — use `make test-bg-status` in a
...
| Single test file | `make test-bg TESTFILE=...` + `make test-bg-status TESTFILE=...` |
| List all running | `make test-bg-list` |
| Kill a stuck test | `make test-bg-kill TESTFILE=...` |
| Kill all background | `make test-bg-kill-all` |
