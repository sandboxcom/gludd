...
function ratchetHasEntries(): boolean {
  try {
    const ratchetPath = path.join(process.cwd(), "config", "ratchet.yml")
    if (!fs.existsSync(ratchetPath)) return false
    const ratchetLines = fs.readFileSync(ratchetPath, "utf8")
      .split(/\r?\n/)
      .filter(line => {
        const trimmed = line.trim()
        return trimmed !== "" && !trimmed.startsWith("#")
      })
    return ratchetLines.length > 0
  } catch {
    return false
  }
}

function gateHasRequiredPasses(): boolean {
  try {
    ...
    return (
      content.includes("lint PASS") &&
      content.includes("typecheck PASS") &&
      content.includes("collect PASS") &&
...
const defaultImpl: HotModule = {
    "tool.execute.before": async (input, output) => {
      ...
      "text.complete": async (_input, output) => {
        // Compatibility hook: delegates to the same gate-status / stop-pattern
        // logic as the canonical text-complete hook for harnesses that still inspect
        // or invoke the legacy hook name.
        if (process.env.OPENCODE_SUBAGENT === "1") return output
        if (_bashPolicyNudge && typeof output !== "string") return output
      ...
      "experimental.text.complete": async (_input, output) => {
        ...
        if (detectStopPattern(output) && (ratchetHasEntries() || !gateHasRequiredPasses())) {
          return STOP_PATTERN_BLOCK + "\n\n" + output
        }
