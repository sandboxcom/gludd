...
> → design: WAVE_C_DESIGNS_2026-07-10.md §C-SPD1, status: DESIGN-COMPLETE (turnkey re-spec of this same finding — supersedes the WAVE_D pointer above; flush phase + `PHASE_ORDER` length 17→18, all 4 phase-count test-file updates enumerated).
...
> → design: WAVE_C_DESIGNS_2026-07-10.md §C-INTEGRITY, status: DESIGN-COMPLETE — covers only the non-canonical-HMAC/field-injection piece (`_canonical_sig_payload` + scheme-tag domain separation across `sign_change`/`verify_signature`/`sign_change_openbao`/`verify_openbao_signature`; fail-closed migration, no accept-both fallback). The unsigned-baseline + corrupt-store-fail-closed pieces (H1/H2) are NOT covered by this design — still open.
...
> → design: WAVE_C_DESIGNS_2026-07-10.md §C-GATEWAY, status: DESIGN-COMPLETE — covers M1 (timeout: `DEFAULT_MODEL_*_TIMEOUT_S` constants + `_build_timeout_kwarg` per provider family) and M3 (alias leak: exception text carries `profile.api_base_alias` only, resolved URL to `logger.debug`). The caller-kwargs-override-SSRF-validated-`base_url`/`api_key` issue is NOT covered by this design — still open.
...
> → design: WAVE_C_DESIGNS_2026-07-10.md §C-RELOAD, status: DESIGN-COMPLETE — covers the `hot_reloader.py` TOCTOU/concurrency/debounce piece (H1/M1/M4: single instance-wide non-reentrant non-blocking `threading.Lock` + per-key debounce around `reload()`/`reload_code_module()`/`reload_changed_modules()`) and the `capability_lattice.py` symlink/`..` bypass (M3, cross-referenced there as C14). The **unauthenticated-worker-registration PSK leak** (`worker_broadcast.py`, highest-severity item in this family) is NOT covered by this design — still open, highest priority residual.
...
> → design: WAVE_C_DESIGNS_2026-07-10.md §C-ENGINE, status: DESIGN-COMPLETE — covers all four: item 3 (delete dead sync `execute()` 600-755 + resurrection guard, migrate 19 test call sites to `asyncio.run(engine.execute_async(job))`), items 15+16 (`defer_commit` returns the `asyncio.Task`, awaited before `_run_tests` runs via `asyncio.to_thread`), item 17 (`_drain_background_tasks` + `shutdown()`, with the flagged wiring gap that `daemon.py` has no shutdown hook calling it yet — that wiring is itself unreached work).
...
> → design: WAVE_C_DESIGNS_2026-07-10.md §C-SELFIMP, status: DESIGN-COMPLETE — covers only the `POST /admin/self-improve/run` code-tier bypass (`routers/self_improve.py:343-406`; new `_code_tier_apply`/`_enqueue_code_change`/`_apply_approved_code_change` trio mirroring the config-tier gate, fail-closed 503 with no session factory). `auto_queue=True`, the human-approval-dead-code path, and the `allow_auto_promote` backdoor are NOT covered by this design section — `allow_auto_promote` is tracked separately as a LOW dead-knob in the design doc's still-in-flight list (design NOT complete for that sub-item); the other two remain open.
...
> → design: WAVE_C_DESIGNS_2026-07-10.md §C-SEC-1 + §C-SEC-1b + §C-RELOAD (M3/C14 subsection), status: DESIGN-COMPLETE for 3 of 4 sub-items — denied grants not enforced (§C-SEC-1: subtract `denied_union` in `intersection()`, add deny-check-before-allow-check gates in `_enforce_permission()`/`StsIssuer.validate()`), STS re-delegation TTL escalation (§C-SEC-1b: clamp stored spec's `max_sts_ttl_seconds` in `issue()`, ceiling check in `is_subset()`), and the symlink/`..` lexical-bypass flavor of deny-list drift in `capability_lattice.py`'s `is_protected_path` (§C-RELOAD M3, via `os.path.realpath` resolution before segment checks). The `_intersect_constraints` widens-file-scope sub-item (`security/permissions.py:514`) is NOT covered by this design wave — still open. Also flagged-not-fixed by §C-RELOAD: `is_collections_path` and `self_update/apply.py:132-142 _is_hard_denied` share the same lexical gap (residual, C9 territory).
...
> → design: WAVE_C_DESIGNS_2026-07-10.md §C-TOOLLOOP, status: DESIGN-COMPLETE for 3 of 4 sub-items — per-response tool-call cap (item 10: `MAX_TOOL_CALLS_PER_RESPONSE = 20`, drift-guard vs `dispatch.py`'s `MAX_CALLS_PER_REQUEST`), args-vs-`input_schema` validation (item 11: `jsonschema.Draft202012Validator`, reject-before-`call_tool`), and VariableStore `"last"` key injection (item 22: `_RESERVED_DISPATCH_NAMES` + `_safe_dispatch_name`). The "capability lattice bypassed on Phase-2 loop" sub-item is NOT covered by this design — still open.
...
> → design: WAVE_C_DESIGNS_2026-07-10.md §C-GITAUTO (GA-1), status: DESIGN-COMPLETE for the #63 sub-item — routes `init_repo`/`create_worktree`/`remove_worktree`/`list_worktrees`/`create_release_tag`/`create_checkpoint_tag`/`create_local_bare_mirror`/`push_to_remote` through `self._run_git(..., _cwd=...)` so all eight sites pick up `git_repo_lock` serialization + `_GIT_TIMEOUT_SECONDS`. `merge_branch`'s lock+timeout bypass (GA-3) and the squash `check=False` fail-open and #64 branch-name collision are NOT covered by this design — still open.
> **New finding from the design doc (not previously tracked here) — GA-tag-dash (LOW):** `tag_release` (`repo.py:271-273`) and `tag_checkpoint` (`repo.py:275-277`) lack `_reject_leading_dash` + `--` end-of-options separator, unlike the rest of the git_automation argv-injection guards. → design: WAVE_C_DESIGNS_2026-07-10.md §C-GITAUTO (GA-tag-dash), status: DESIGN-COMPLETE — `_reject_leading_dash(tag, kind="tag name")` + reorder to `self._run_git("tag","-a","-m",f"Release {tag}","--",tag)` / `self._run_git("tag","--",tag)`.
...
> **C31 (P2) — `TodoModel` unbounded `Text` columns (D-07-b). New item — not previously tracked in this section; added from the Wave C design sweep.**
> Evidence: 13 unbounded `Text` columns on `TodoModel` (`db/models.py`) have no length/size bound, unlike the sibling fix already landed for `task_decisions`/`audit_events` (migration `026`, commit `1c6afab0` "D-07 bound unbounded Text blob columns with 64KiB length CheckConstraints... via migration 026 plus parity CHECK-dimension" — verify this landed commit does NOT already cover `TodoModel`; it does not per the design doc's framing of this as a still-open mirror).
> Fix: a new migration `027` mirroring the `026` CHECK-constraint pattern (64KiB length `CheckConstraint` per column) applied to all 13 `TodoModel` `Text` columns, plus a parity test mirroring `026`'s CHECK-dimension test.
> Test plan: migration-parity unit test (all 13 columns carry the constraint); a write-exceeding-bound integration test asserting the DB rejects rather than silently truncating/accepting unbounded input.
> → design: WAVE_C_DESIGNS_2026-07-10.md "Remaining Wave C items" § C-TODOMODEL, status: IN-FLIGHT (NOT design-complete — the design doc explicitly marks this item "(in flight)", unlike the 9 fully turnkey sections above; only the target pattern (mirror migration 026) is specified, not a line-anchored implementation).
