...
> **A3 — Slurm cost-cap semantics reconciliation.** *(fix drafted in-session — verify landed)*
...
> **A4 — GPU-metrics environment pollution (3.12, shard other).**
...
> **A5 — `tests/unit/test_rg_search.py::test_build_argv_drops_disallowed_flags`.**
> `assert False`, deterministic both pythons. Source: the `build_argv` flag-allowlist in `src/general_ludd/code_intelligence/` (grep `build_argv`). Security-relevant (disallowed rg flags must be dropped) — find the real regression or ordering cause; never weaken. Treat as UNFIXED regardless of old commit messages claiming otherwise.
...
> **A7 — unit-1a shard cancellation (both pythons).**
...
> **A8 — build.yml coverage job can red the pipeline despite "non-gating" intent.**
...
> **A9 — One test file silently dropped from CI.**
...
> Evidence: build.yml:126/321 comments say "~629 test files"/"86 sequential scenarios"; reality is 1214 files / 113 scenarios, so each of 4 molecule shards runs ~28 scenarios against a 15-minute `timeout-minutes` (build.yml:320) — timeout-cancellation risk and wasted minutes.
> Fix: update comments to real counts; measure a shard's wall time from a recent run; raise `timeout-minutes` or go to 6 shards.
...
> Also fix the cosmetic deck-build warning "tokens not found in template: {{VERSION}}…" — `scripts/build_deck.py --build` warns because the tracked `docs/presentation/deck/index.html` already has baked values; make the regeneration idempotent (replace by anchored spans/comment markers, not one-shot `{{TOKEN}}`s).
...
> **C7 — MCP transport allowlist (M2).** [RE-VERIFY]
> `src/general_ludd/mcp/transport.py:28`: `python`/`node` allowlisted with no argv validation; some version pins float; `uvx` unpinned. Fix: argv allowlist per interpreter (module/script paths jailed to repo), pin versions.
...
> **C21 — ALPHA4 leftovers: RE-TRIAGED against code 2026-07-10.** Most of the old list is closed — do NOT re-implement the FIXED/MITIGATED entries below; only the three STILL-OPEN bullets are work.
> **FIXED (verified):** alembic logging sections (env.py logger fix landed in local commit `2543152b`); SEC-8 `/api/status` db_url leak; M-3 unknown-role fail-open (`resolve_role(strict=True)` at `models/gateway.py:1360`); `record_success` fallback double-count; SEC-4 webhooks (rewritten to tracked async httpx + redaction, `events/hooks.py:241-301`); M-4 `list_all()` clamp (`db/repository.py:337-365`); SEC-5b `resolve()` now calls `_enforce_permission` (`secrets/manager.py:286-295`).
> **MITIGATED (no action unless listed residual):** M-13 (CAS optimistic lock at update sites — stronger than the audit assumed); M-7 (bounds added; residual: `_sort_by_ts` NaN/Inf sort key at `connectors/base.py:303-309`, LOW — fix with a finite-key guard); skills fetcher (RemoteSkillFetcher has a 1MB cap; residual: `GitHubSkillFetcher` at `skills/fetcher.py:109,:130,:133` uncapped, LOW-MED — add the same cap); prompt_registry refresh moved to `to_thread`; alembic URL has env override; M-10 circuit-breaker per-process state is fine under the enforced single-worker clamp (revisit in D19).
> **NON-ISSUE:** M-5 `/docs` prefix (bad repro).
> **STILL OPEN (real work):**
> - `validation/runner.py:31-45` — subprocess not symlink-confined post-validation, and its caller `reload/self_improve.py:84-89` doesn't confine either. Fix: resolve+jail the workdir before exec.
> - `event_loop/loop.py:1330-1361` — PID/concurrency cap applied AFTER rows are marked ACTIVE (over-cap todos sit ACTIVE-without-dispatch in the window). Fix: cap check before the CAS claim, or requeue-on-cap in the same transaction.
> - `event_loop/loop.py:1031-1035` — `_dispatch_review_job` `to_thread(run_playbook)` with no timeout. Fix: `asyncio.wait_for` with a config-driven review timeout + BLOCKED transition on expiry.
...
> **D17 (P2) — Failover xfail gaps.**
> `docs/audit/FAILOVER_GAPS.md`: 6b all-down structured error; 12 correlation-ID propagation; 13a fallback concurrency cap; 14b failover_count facet. Turn each xfail into a pass.
...
> Evidence (code-verified via coverage map): `src/general_ludd/routers/remediation.py` (~319 lines) and `src/general_ludd/routers/eval.py` (~87 lines) are never registered — not in `routers/__init__.py::register_all` and not directly in `daemon.py` (contrast: `self_update.register(app, daemon_state)` at daemon.py:2811). Their endpoints are unreachable in production while looking finished in the tree.
> Fix: decide per router — register in `register_all` (with PSK auth + tests, and for remediation.py land C25's idempotency guard in the same change) or delete with a TASKS.md rationale. Auto-remediation (#52 / D9) likely depends on remediation.py being reachable — coordinate.
...
> Evidence: `docs/audit/E2E_AUDIT_2026-07-06.md` — 16 failing e2e (15 in `tests/e2e/test_environment_e2e.py`) [RE-VERIFY — e2e shard isn't red on the latest run, so these may be skipped or since-fixed], ~40 src modules with zero e2e coverage. Fix failing ones first; then add e2e for the top-10 riskiest uncovered modules (daemon lifecycle, reconcile, review).
...
> Eight src modules have NO test references at all — write unit suites in this priority order:
...
> 3. `src/general_ludd/routers/remediation.py` (~319) and 4. `src/general_ludd/routers/eval.py` (~87) — test as part of the D21 wire-or-delete decision.
> 5. `src/general_ludd/runtime/release_orchestrator.py` (60, release composition point). 6. `src/general_ludd/renderers/cache.py` (63, TTL expiry logic). 7. `src/general_ludd/event_loop/benchmark.py` (42). 8. `src/general_ludd/renderers/executor.py` (17, shim).
...
> Ten routers are touched ONLY by the generic registration smoke test — no endpoint-level tests. Write FastAPI TestClient endpoint tests (happy path + auth-required + validation error per endpoint), priority order: `routers/security.py` (666 lines, STS/permission admin — worst AND security-sensitive), `ornith.py` (323), `account.py` (212 — its docstring falsely claims router coverage; fix the docstring too), `adversarial.py` (149 — coordinate with C2's confinement tests), `benchmark.py` (130), `model_performance.py` (119), `memory.py` (94), `mcp.py` (67), `worktree.py` (60), `variants.py` (36).

> **E9 — Skip-smell cleanup.**
> - Hook-liveness guardrail tests (~30 sites) are `skipif CI=="true"` — the guardrail-liveness suite is disabled exactly where it matters. Make them CI-runnable (hermetic fixtures) or convert to an explicit opt-in marker with a documented local runbook.
...
> Note: a dedicated design for making the hook-liveness suite CI-runnable is incoming — hold the ~30-site `skipif CI` cleanup until it lands, then implement per that design.
...
**Priority totals:** P0: 14 (A0-A10 + A6-discipline, B1, B2, D1) · P1: 41 (A11, A12, C1-C26 family, D2, D3, D4, D7.1-D7.3, D9-D11, D21, E1-E4, E6-E11, F1-F4) · P2: 14 (D5, D6, D7.4, D12-D20, E5, E12, F5). Of these, the §3.0.1 landed-locally set (A1-A4, A7, A9, A10, C1-most, C2, C3, D1, parts of C21/C26/F1) awaits only push + CI confirmation.
