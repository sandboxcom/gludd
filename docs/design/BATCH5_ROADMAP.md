...
- **2.3** Add the missing `gludd_*` module surfaces for the Wave-1 subsystems (thin shims; daemon-HTTP where a single-writer/router boundary applies):
  - **2.3a** `gludd_observe` (HIGH — unblocks 7 `observe_*` roles that call a non-existent module; removes every "DEFERRED WIRING #73" comment). doc: `tool_calls_via_ansible.md M3` + `observe_debugging_roles.md §3.1`. ops: `query`/`correlate`/`timeline`/`topology`/`sources`. Copy the `gludd_db.py` skeleton (capability gate → `GluddClient` → endpoint). files: NEW `collections/.../plugins/modules/gludd_observe.py`. deps: Wave 1 §1C.2/§1D.1 (observe endpoints live). size: M.
