...
| PSK-flat authz cluster | A-EXEC-ROUTES-PSK-ONLY, A-PERMSPEC-SELF-EDIT, A-STS-REVOKE-ANY, A-DEPLOY-DESTROY-ANY, A-ACCOUNT-ANY: every PSK holder (== every dispatched agent) has full mutating admin | per-route capability check against the agent-permission lattice; ownership column on permission specs/tokens/deployments | L |
