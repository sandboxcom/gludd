...
def attach_module_utils_path(role_path: str | None = None) -> None:
    if role_path is None:
        role_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    utils_path = os.path.join(role_path, "..", "..", "plugins", "module_utils")
...
def signal_identify(
            ...
            if estimated_bandwidth is None or estimated_shape == "":
                ...
                if estimated_shape == "":
                    ...
                    if len(mean_spectrum) > 0 and np.max(mean_spectrum) > 0:
                        ...
                        num_peaks = max(np.sum(transitions > 0), 0)
                        if num_peaks == 0:
    ...
    attach_module_utils_path()
    from modulation_schemes import classify_signal
