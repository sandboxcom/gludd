...
# Keep blocking model traffic independent from asyncio's process-wide default
# executor.  That pool also carries unrelated filesystem, subprocess, and test
# work; when those workers are occupied, even an instantaneous model call can
# sit queued until the tool-loop timeout fires.  A single namespaced pool bounds
# resource use across every ToolCallLoop instance while preserving contextvars.
...
async def _run_model_call(
    ...
    **kwargs: Any,
    ...
    """Run one blocking gateway call without depending on the default executor."""
