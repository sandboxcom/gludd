...
class ProcessExecutor:
    ...
    def _apply_limits(limits: ProcessLimits) -> None:
        import resource
        def _set(resource_id: int, soft: int, hard: int) -> None:
            try:
        ...
        if limits.memory_mb is not None:
            ...
            _set(resource.RLIMIT_AS, memory_bytes, memory_bytes)
        if limits.cpu_seconds is not None:
            _set(resource.RLIMIT_CPU, limits.cpu_seconds, limits.cpu_seconds)
        if limits.max_file_size is not None:
            _set(resource.RLIMIT_FSIZE, limits.max_file_size, limits.max_file_size)
        if limits.max_open_files is not None:
            _set(resource.RLIMIT_NOFILE, limits.max_open_files, limits.max_open_files)
