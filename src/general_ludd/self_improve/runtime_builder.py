...
def _configure_live_candidate_wiring(
    ...
    if self_improve_config is not None:
        # Runtime APIs are stable patch points for plugins and tests. Filter newer
        # optional evidence inputs so older trusted adapters remain callable.
        configured = invoke_with_supported_keywords(
            runtime_api.build_azure_containerapp_bootstrap_wiring,
            (canonical_root, self_improve_config),
            {
...
def build_managed_self_improve_runner(
    ...
    *,
    ...
    configured_evidence_store = configured_capability_evidence_store(
        self_improve_config
    )
