...
def check_python_coverage(
    ...
    py = cast(dict[str, object], config.get("python", {}))
    target_line = cast(float, py.get("line_coverage_min_percent", 90.0))
    target_branch = cast(float, py.get("branch_coverage_min_percent", 80.0))
...
def check_molecule_coverage(config: dict[str, object]) -> MoleculeCoverageReport:
    registry = cast(dict[str, str], config.get("playbook_registry", {}))
    roots = cast(list[str], config.get("scenario_roots", []))
