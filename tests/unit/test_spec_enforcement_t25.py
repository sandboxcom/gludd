...
class TestT25BackgroundGatePhaseMarkers:
    ...
    def test_gate_background_target_exists(self):
        content = MAKEFILE.read_text()
        assert "\ngate-background:" in content, "T25: gate-background target must exist in Makefile"

    def test_gate_background_writes_to_gate_logs(self):
        content = MAKEFILE.read_text()
        idx = content.find("\ngate-background:")
        assert idx != -1
        end = content.find("\n\n", idx)
    ...
    def test_gate_target_has_minimum_phase_markers(self):
        content = MAKEFILE.read_text()
        idx = content.find("\ngate:")
        assert idx != -1
    ...
    def test_gate_lite_has_minimum_phase_markers(self):
        content = MAKEFILE.read_text()
        idx = content.find("\ngate-lite:")
    ...
    def test_gate_status_check_reports_phase(self):
        content = MAKEFILE.read_text()
        idx = content.find("\ngate-status-check:")
    ...
    def test_gate_background_has_timeout_protection(self):
        content = MAKEFILE.read_text()
        idx = content.find("\ngate-background:")
        assert idx != -1
        end = content.find("\n\n", idx)
