...
class TestMethodNotAllowed:
    ...
    def test_options_returns_405(self, client):
        resp = client.options("/api/terraform/state/s")
        assert resp.status_code == 405

    def test_head_returns_405(self, client):
        resp = client.head("/api/terraform/state/s")
        assert resp.status_code == 405

    def test_custom_method_returns_405(self, client):
...
class TestLockDefaults:
    def test_missing_id_generates_valid_uuid(self, client):
        resp = client.request("LOCK", "/api/terraform/state/s", json={})
        assert resp.status_code == 200
        uid = resp.json()["ID"]
        uuid.UUID(uid)

    def test_empty_string_id_generates_valid_uuid(self, client):
        resp = client.request("LOCK", "/api/terraform/state/s", json={"ID": ""})
        assert resp.status_code == 200
        uid = resp.json()["ID"]
        uuid.UUID(uid)

    def test_missing_operation_defaults_empty(self, client):
        resp = client.request("LOCK", "/api/terraform/state/s", json={"ID": "x"})
        assert resp.json()["Operation"] == ""

    def test_missing_info_defaults_empty(self, client):
        resp = client.request("LOCK", "/api/terraform/state/s", json={"ID": "x"})
        assert resp.json()["Info"] == ""

    def test_missing_who_defaults_empty(self, client):
        resp = client.request("LOCK", "/api/terraform/state/s", json={"ID": "x"})
        assert resp.json()["Who"] == ""

    def test_missing_version_defaults_empty(self, client):
        resp = client.request("LOCK", "/api/terraform/state/s", json={"ID": "x"})
        assert resp.json()["Version"] == ""

    def test_missing_created_defaults_to_iso_now(self, client):
        resp = client.request("LOCK", "/api/terraform/state/s", json={"ID": "x"})
    ...
    def test_missing_path_defaults_empty(self, client):
        resp = client.request("LOCK", "/api/terraform/state/s", json={"ID": "x"})
        assert resp.json()["Path"] == ""

    def test_all_seven_lock_fields_returned(self, client):
        resp = client.request("LOCK", "/api/terraform/state/s", json={"ID": "x"})
        data = resp.json()
...
class TestUnlockEdgeCases:
    def test_unlock_empty_provided_id_on_empty_lock(self, client):
        resp = client.request("UNLOCK", "/api/terraform/state/nolock", json={})
        assert resp.status_code == 200
        assert resp.json() == {}

    def test_unlock_provided_empty_string_allowed_when_lock_exists(self, client):
        client.request("LOCK", "/api/terraform/state/s", json={"ID": "lock"})
        resp = client.request("UNLOCK", "/api/terraform/state/s", json={"ID": ""})
        assert resp.status_code == 200

    def test_unlock_after_double_unlock_is_idempotent(self, client):
        client.request("LOCK", "/api/terraform/state/s", json={"ID": "lock"})
    ...
    def test_unlock_substring_of_lock_id_is_rejected(self, client):
...
class TestLockConflict:
    def test_423_response_contains_original_lock_data(self, client):
    ...
    def test_423_response_preserves_all_stored_fields(self, client):
        lock_body = {
            "ID": "orig",
            "Operation": "apply",
            "Info": "info",
...
class TestUnlock409Detail:
    def test_409_detail_contains_both_ids(self, client):
    ...
    def test_409_detail_mentions_lock_id_mismatch(self, client):
...
class TestStateDeep:
    ...
    def test_post_three_times_keeps_last(self, client):
        client.post("/api/terraform/state/s", json={"v": 1})
    ...
    def test_post_empty_body_stores_empty_dict(self, client):
        resp = client.post("/api/terraform/state/s", json={})
        assert resp.status_code == 200
        assert client.get("/api/terraform/state/s").json()["state"] == {}

    def test_delete_also_removes_state_from_store(self, client):
        ts_module._state_store["s"] = {"v": 1}
        client.delete("/api/terraform/state/s")
        assert "s" not in ts_module._state_store

    def test_delete_also_removes_lock_from_store(self, client):
...
class TestRouteRegistration:
    def test_operation_ids_set_on_routes(self, client):
        app = FastAPI()
        ts_module.register(app, {})
        ops = []
        for r in app.routes:
    ...
    def test_register_is_idempotent_routes_double(self, client):
        app = FastAPI()
        ts_module.register(app, {})
        ts_module.register(app, {})
    ...
    def test_register_passes_daemon_state_unchanged(self, client):
        app = FastAPI()
...
class TestRelockAfterDelete:
    def test_lock_delete_lock_succeeds(self, client):
    ...
    def test_post_lock_delete_post_relock(self, client):
        client.post("/api/terraform/state/s", json={"v": 1})
        client.request("LOCK", "/api/terraform/state/s", json={"ID": "a"})
        client.delete("/api/terraform/state/s")
        client.post("/api/terraform/state/s", json={"v": 2})
...
class TestMultiStackSerial:
    def test_five_stacks_independent(self, client):
        for i in range(5):
...
class TestPostResponse:
    def test_post_returns_empty_dict(self, client):
        resp = client.post("/api/terraform/state/s", json={"v": 1})
        assert resp.json() == {}

    def test_post_overwrite_returns_empty_dict(self, client):
        client.post("/api/terraform/state/s", json={"v": 1})
        resp = client.post("/api/terraform/state/s", json={"v": 2})
        assert resp.json() == {}

    def test_delete_returns_empty_dict(self, client):
        client.post("/api/terraform/state/s", json={"v": 1})
