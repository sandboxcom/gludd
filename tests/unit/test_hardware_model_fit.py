...
class TestGpuInfoToGpuTable:
    ...
    def test_name_to_table_key_internal(self) -> None:
...
class TestCanRunModel:
    ...
    def test_llama_8b_on_h100_fits(self) -> None:
        inv = HardwareInventory(
            gpus=[GpuInfo(name="NVIDIA H100", vram_gb=80.0, backend="nvidia")],
            total_ram_gb=256.0,
            disk_free_gb=500.0,
    ...
    def test_mistral_7b_on_t4_fits(self) -> None:
        inv = HardwareInventory(
    ...
    def test_llama_70b_on_t4_fails(self) -> None:
        inv = HardwareInventory(
            gpus=[GpuInfo(name="Tesla T4", vram_gb=16.0, backend="nvidia")],
            total_ram_gb=64.0,
        )
    ...
    def test_deepseek_v3_on_8x_h200(self) -> None:
        inv = HardwareInventory(
    ...
    def test_phi3_mini_on_rtx_4090(self) -> None:
        inv = HardwareInventory(
    ...
    def test_gemma_2b_on_metal(self) -> None:
        inv = HardwareInventory(
    ...
    def test_llama_405b_on_single_h100_fails(self) -> None:
        inv = HardwareInventory(
            gpus=[GpuInfo(name="NVIDIA H100", vram_gb=80.0, backend="nvidia")],
        )
    ...
    def test_quant_ladder_prefers_higher_quality(self) -> None:
        inv = HardwareInventory(
            gpus=[GpuInfo(name="NVIDIA H100", vram_gb=80.0, backend="nvidia")],
        )
    ...
    def test_stripped_model_name(self) -> None:
        inv = HardwareInventory(
            gpus=[GpuInfo(name="Tesla T4", vram_gb=16.0, backend="nvidia")],
        )
        r = can_run_model(inv, "  mistral-7b  ")
    ...
    def test_multi_gpu_aggregates_vram(self) -> None:
        inv = HardwareInventory(
            gpus=[
...
class TestExtractModelParams:
    def test_standard_b_suffix(self) -> None:
    ...
    def test_standard_b_suffix_variants(self) -> None:
    ...
    def test_moe_pattern_deepseek(self) -> None:
    ...
    def test_moe_detection_from_name(self) -> None:
        result = _extract_model_params("some-moe-model-8b")
        assert result is not None
        assert result["params_b"] == 8.0
        assert result["is_moe"] is True

    def test_mixtral_anywhere_detected_as_moe(self) -> None:
...
class TestUnifiedProbe:
    ...
    def test_falls_back_to_metal(self) -> None:
        survey = HardwareSurvey()
        gpu = GpuInfo(name="Apple M3 Max", vram_gb=48.0, backend="metal")
        with (
    ...
    def test_falls_back_to_rocm(self) -> None:
        survey = HardwareSurvey()
        gpu = GpuInfo(name="AMD GPU 0", vram_gb=24.0, backend="rocm")
        with (
            mock.patch.object(survey, "probe_gpu_nvidia", return_value=[]),
    ...
    def test_empty_when_no_gpu(self) -> None:
        survey = HardwareSurvey()
        with (
            mock.patch.object(survey, "probe_gpu_nvidia", return_value=[]),
            mock.patch.object(survey, "probe_gpu_metal", return_value=[]),
    ...
    def test_default_survey_creates_instance(self) -> None:
