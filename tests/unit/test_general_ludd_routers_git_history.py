...
class TestGitHistorySearch:
    def test_search_returns_empty_list(self, client, mock_indexer):
        mock_indexer.search.return_value = []
        response = client.get("/api/git/history?q=test")
        assert response.status_code == 200
        assert response.json() == []

    def test_search_with_all_params(self, client, mock_indexer):
        response = client.get(
            "/api/git/history",
            params={
                "q": "fix",
    ...
    def test_search_default_params(self, client, mock_indexer):
        response = client.get("/api/git/history")
        assert response.status_code == 200
        mock_indexer.search.assert_called_once()

    def test_search_limit_below_1_returns_422(self, client):
        response = client.get("/api/git/history?limit=0")
        assert response.status_code == 422

    def test_search_limit_above_500_returns_422(self, client):
    ...
    def test_search_handles_exception(self, client, mock_indexer):
...
class TestGitHistoryStats:
    def test_stats_returns_dict(self, client, mock_indexer):
    ...
    def test_stats_handles_exception(self, client, mock_indexer):
...
class TestGitHistoryReindex:
    def test_reindex_returns_ok(self, client, mock_indexer):
    ...
    def test_reindex_handles_exception(self, client, mock_indexer):
