...
def test_cache_write_and_read(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Writing a cache entry then reading it back returns the same count."""
    cache_file = tmp_path / "cache.json"
    monkeypatch.setattr(agent_liveness, "CACHE_FILE", str(cache_file))
    agent_liveness._write_cache(7)
    cached = agent_liveness._read_cache()
...
def test_cache_missing_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """Reading a nonexistent cache file returns None."""
    cache_file = tmp_path / "nonexistent_cache.json"
    monkeypatch.setattr(agent_liveness, "CACHE_FILE", str(cache_file))
    assert agent_liveness._read_cache() is None


def test_cache_corrupted_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
