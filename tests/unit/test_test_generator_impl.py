"""Tests for TestGeneratorImpl — creates pytest files from TestSpec."""
...
from general_ludd.agents.test_generation.contracts import TestHarness, TestSpec
from general_ludd.agents.test_generation.test_generator import TestGeneratorImpl


class TestGeneratorImplCreation:
    def test_creates_with_spec_and_harness(self) -> None:
        spec = TestSpec(target_module="general_ludd.foo")
        harness = TestHarness()
        gen = TestGeneratorImpl(spec=spec, harness=harness)
        assert gen.spec is spec
        assert gen.harness is harness

    def test_default_output_dir_from_spec(self) -> None:
        spec = TestSpec(target_module="general_ludd.foo", output_dir="tests/e2e")
        gen = TestGeneratorImpl(spec=spec, harness=TestHarness())
        assert gen.spec.output_dir == "tests/e2e"


class TestGeneratorImplGenerateFile:
