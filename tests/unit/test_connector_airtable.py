"""Unit tests for the self-contained Airtable record-list connector.

All HTTP is mocked through an injected fake transport — no network access.
"""

from __future__ import annotations

from typing import Any

import pytest

from general_ludd.connectors.airtable import AirtableSource

BASE_ID = "appXXXXXXXXXXXXXX"
TABLE_NAME = "tblXXXXXXXXXXXXXX"
TOKEN_ENV = "AIRTABLE_TEST_TOKEN"


class FakeResponse:
    def __init__(
        self,
        status_code: int = 200,
        body: Any = None,
        headers: dict[str, str] | None = None,
    ) -> None:
        self.status_code = status_code
        self._body = body if body is not None else {}
        self.headers = headers or {}

    def json(self) -> Any:
        return self._body


class RecordingTransport:
    """Replays a queue of responses and records every call."""

    def __init__(self, responses: list[FakeResponse]) -> None:
        self._responses = list(responses)
        self.calls: list[dict[str, Any]] = []

    def __call__(
        self,
        method: str,
        url: str,
        *,
        headers: dict[str, str] | None = None,
        params: dict[str, str] | None = None,
        timeout: float = 30.0,
    ) -> FakeResponse:
        self.calls.append(
            {
                "method": method,
                "url": url,
                "headers": headers or {},
                "params": params or {},
                "timeout": timeout,
            }
        )
        if not self._responses:
            return FakeResponse(200, {})
        return self._responses.pop(0)


def _raising_transport(*args: Any, **kwargs: Any) -> FakeResponse:
    raise ConnectionError("boom")


CANNED_RESPONSE = {
    "records": [
        {
            "id": "rec001",
            "createdTime": "2026-07-01T10:00:00.000Z",
            "fields": {"Name": "Alice", "Status": "Active"},
        },
        {
            "id": "rec002",
            "createdTime": "2026-07-01T10:05:00.000Z",
            "fields": {"Name": "Bob", "Status": "Inactive"},
        },
    ],
}


@pytest.fixture
def token(monkeypatch: pytest.MonkeyPatch) -> str:
    monkeypatch.setenv(TOKEN_ENV, "s3cr3t-airtable-token")
    return "s3cr3t-airtable-token"


def _src(transport: Any, **extra: Any) -> AirtableSource:
    cfg: dict[str, Any] = {
        "base_id": BASE_ID,
        "table_name": TABLE_NAME,
        "token_env": TOKEN_ENV,
    }
    cfg.update(extra)
    return AirtableSource(cfg, transport=transport)


# -- contract / attributes ------------------------------------------------


def test_class_attrs() -> None:
    assert AirtableSource.KIND == "records"
    src = AirtableSource(
        {"base_id": BASE_ID, "table_name": TABLE_NAME, "token_env": TOKEN_ENV},
        transport=lambda *a, **k: None,
    )
    assert src.name == "airtable"


def test_requires_base_id() -> None:
    with pytest.raises(ValueError, match="base_id"):
        AirtableSource(
            {"table_name": TABLE_NAME, "token_env": TOKEN_ENV},
            transport=lambda *a, **k: None,
        )


def test_requires_table_name() -> None:
    with pytest.raises(ValueError, match="table_name"):
        AirtableSource(
            {"base_id": BASE_ID, "token_env": TOKEN_ENV},
            transport=lambda *a, **k: None,
        )


def test_requires_token_env() -> None:
    with pytest.raises(ValueError, match="token_env"):
        AirtableSource(
            {"base_id": BASE_ID, "table_name": TABLE_NAME},
            transport=lambda *a, **k: None,
        )


# -- normalization --------------------------------------------------------


def test_query_normalizes(token: str) -> None:
    transport = RecordingTransport([FakeResponse(200, CANNED_RESPONSE)])
    rows = _src(transport).query({"maxRecords": "10"})

    assert len(rows) == 2
    first = rows[0]
    assert first["ts"] == "2026-07-01T10:00:00.000Z"
    assert first["source"] == "airtable"
    assert first["kind"] == "records"
    assert first["value"] == 1
    assert first["labels"]["record_id"] == "rec001"
    assert first["raw"]["id"] == "rec001"
    assert rows[1]["labels"]["record_id"] == "rec002"


def test_query_passes_spec_params(token: str) -> None:
    transport = RecordingTransport([FakeResponse(200, {"records": []})])
    _src(transport).query(
        {"filterByFormula": "{Status}=Active", "view": "Grid View"}
    )
    sent = transport.calls[0]["params"]
    assert sent["filterByFormula"] == "{Status}=Active"
    assert sent["view"] == "Grid View"


# -- auth from env --------------------------------------------------------


def test_auth_header_present(token: str) -> None:
    transport = RecordingTransport([FakeResponse(200, {"records": []})])
    _src(transport).query({})
    assert transport.calls[0]["headers"]["Authorization"] == f"Bearer {token}"


def test_missing_env_token_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv(TOKEN_ENV, raising=False)
    transport = RecordingTransport([FakeResponse(200, {"records": []})])
    with pytest.raises(RuntimeError, match="unset or empty"):
        _src(transport).query({})


def test_token_not_in_any_url_or_params(token: str) -> None:
    transport = RecordingTransport([FakeResponse(200, CANNED_RESPONSE)])
    _src(transport).query({})
    call = transport.calls[0]
    assert token not in call["url"]
    assert token not in str(call["params"])


# -- pagination (offset-based) --------------------------------------------


def test_pagination_follows_offset(token: str) -> None:
    page1 = FakeResponse(
        200,
        {"records": [CANNED_RESPONSE["records"][0]], "offset": "itr_abc123"},
    )
    page2 = FakeResponse(200, {"records": [CANNED_RESPONSE["records"][1]]})
    transport = RecordingTransport([page1, page2])
    rows = _src(transport).query({})
    assert len(rows) == 2
    assert len(transport.calls) == 2
    assert transport.calls[1]["params"].get("offset") == "itr_abc123"


def test_pagination_bounded_by_max_pages(token: str) -> None:
    looping = FakeResponse(
        200,
        {"records": [CANNED_RESPONSE["records"][0]], "offset": "itr_looping"},
    )
    transport = RecordingTransport([looping] * 50)
    rows = _src(transport, max_pages=3).query({})
    assert len(transport.calls) == 3
    assert len(rows) == 3


# -- SSRF -----------------------------------------------------------------


@pytest.mark.parametrize(
    "bad",
    [
        "http://127.0.0.1",
        "https://localhost",
        "http://10.0.0.5",
        "http://169.254.169.254",
        "http://192.168.1.1",
        "http://[::1]",
    ],
)
def test_ssrf_rejects_private(bad: str) -> None:
    with pytest.raises(ValueError, match=r"private|loopback"):
        AirtableSource(
            {
                "base_id": BASE_ID,
                "table_name": TABLE_NAME,
                "token_env": TOKEN_ENV,
                "api_url": bad,
            },
            transport=lambda *a, **k: None,
        )


def test_ssrf_allow_private_opt_in(token: str) -> None:
    transport = RecordingTransport([FakeResponse(200, {"records": []})])
    src = AirtableSource(
        {
            "base_id": BASE_ID,
            "table_name": TABLE_NAME,
            "token_env": TOKEN_ENV,
            "api_url": "http://127.0.0.1",
            "allow_private": True,
        },
        transport=transport,
    )
    assert src.query({}) == []


def test_rejects_non_http_scheme() -> None:
    with pytest.raises(ValueError, match="scheme"):
        AirtableSource(
            {
                "base_id": BASE_ID,
                "table_name": TABLE_NAME,
                "token_env": TOKEN_ENV,
                "api_url": "file:///etc/passwd",
            },
            transport=lambda *a, **k: None,
        )


# -- health ---------------------------------------------------------------


def test_health_ok(token: str) -> None:
    transport = RecordingTransport([FakeResponse(200, {"records": []})])
    h = _src(transport).health()
    assert h["ok"] is True
    assert "detail" in h


def test_health_not_ok_on_4xx(token: str) -> None:
    transport = RecordingTransport([FakeResponse(403, {"error": "forbidden"})])
    h = _src(transport).health()
    assert h["ok"] is False
    assert h["detail"] == "airtable HTTP 403"


def test_health_never_raises(token: str) -> None:
    src = _src(_raising_transport)
    h = src.health()
    assert h["ok"] is False
    assert h["detail"] == "health check failed"


def test_query_raises_on_http_error(token: str) -> None:
    transport = RecordingTransport([FakeResponse(500, {})])
    with pytest.raises(RuntimeError, match="HTTP 500"):
        _src(transport).query({})


def test_timeout_is_bounded(token: str) -> None:
    transport = RecordingTransport([FakeResponse(200, {"records": []})])
    _src(transport, timeout=5.0).query({})
    assert transport.calls[0]["timeout"] == 5.0
