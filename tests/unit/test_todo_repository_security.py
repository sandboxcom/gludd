...
D-28
----
Security invariant (no DB-primary-key forgery):
  ``id``       — the autoincrement DB primary key — is IMMUTABLE and REJECTED by
                 create().  Accepting it would let callers forge primary keys.

  ``todo_id``  — an APPLICATION-ASSIGNED business identifier (e.g. "TODO-001",
                 generated by the /api/todos router via ``f"TODO-{uuid...}"``).
                 It IS allowed at create() because every legitimate create path
                 (the router and direct repo callers) supplies it.  It is NOT
                 the DB primary key, so accepting it does NOT weaken PK-forgery
                 protection.

The real security property is: ``id`` (the DB PK) cannot be forged.
``todo_id`` acceptance is intentional and mirrors the router/e2e behavior.

- ``create()`` rejects any payload that contains the immutable DB fields (``id``,
  ``version``, ``created_at``, ``updated_at``).
- ``create()`` ACCEPTS ``todo_id`` — it is the app-assigned business key.
...
class TestCreateFieldAllowlist:
    ...
    @pytest.mark.asyncio
    async def test_create_allows_todo_id_field(self, session: AsyncSession):
        """todo_id is the app-assigned business key, NOT the DB PK.
        The router and every legitimate caller supply it at create time
        (e.g. ``f"TODO-{uuid4().hex[:8].upper()}"``) so it must be ACCEPTED.
        The real PK-forgery protection is that ``id`` (the DB autoincrement PK)
        is still rejected — tested separately in test_create_rejects_id_field."""
        repo = TodoRepository(session)
        # Must NOT raise — todo_id is an allowed field at create
        todo = await repo.create({"title": "t", "todo_id": "TODO-APP-ASSIGNED"})
        assert todo.todo_id == "TODO-APP-ASSIGNED"
    ...
    async def test_create_immutable_fields_constant_covers_required_set(self):
        """_IMMUTABLE_FIELDS must contain at minimum the DB-managed fields.

        Security invariant: ``id`` (the autoincrement DB primary key), ``version``,
        ``created_at``, and ``updated_at`` must all be in _IMMUTABLE_FIELDS so
        callers cannot forge the primary key, skip version accounting, or
        back-date timestamps.

        ``todo_id`` is intentionally NOT in this required set — it is the
        app-assigned business identifier, allowed at create() so the router and
        direct callers can supply it (e.g. "TODO-001").  PK-forgery protection
        is provided entirely by ``id`` being rejected.
        """
        required = {"id", "version", "created_at", "updated_at"}
        assert required <= TodoRepository._IMMUTABLE_FIELDS
        # Belt-and-suspenders: the DB PK specifically must always be protected.
        assert "id" in TodoRepository._IMMUTABLE_FIELDS
        # todo_id is the business key and must NOT be blocked.
