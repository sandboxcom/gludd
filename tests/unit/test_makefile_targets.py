...
class TestReleaseCutPipeline:
    """release-cut must enforce require-ci-green (step 0) and verify-release-artifact (final step).

    Per AGENTS.md:
      - "Release Pipeline Must Be CI-Green": require-ci-green is step 0 of release-cut
        and aborts the release if CI is not green.
    ...
    def test_require_ci_green_target_exists(self):
    ...
    def test_release_cut_invokes_require_ci_green(self):
        """require-ci-green must be invoked before any push/tag step."""
        recipe = _recipe("release-cut")
        assert "require-ci-green" in recipe, (
            "release-cut must invoke require-ci-green as step 0 "
            "(AGENTS.md: Release Pipeline Must Be CI-Green)"
        )

    def test_release_cut_require_ci_green_precedes_push(self):
        """require-ci-green must appear in the recipe before git-push-sandboxcom."""
        recipe = _recipe("release-cut")
        ci_pos = recipe.find("require-ci-green")
        push_pos = recipe.find("git-push-sandboxcom")
        assert ci_pos != -1 and push_pos != -1, (
            "release-cut must reference both require-ci-green and git-push-sandboxcom"
        )
        assert ci_pos < push_pos, (
            "release-cut must run require-ci-green BEFORE git-push-sandboxcom "
            "(step 0 ordering per AGENTS.md)"
        )

    def test_release_cut_invokes_verify_release_artifact(self):
    ...
    def test_release_cut_verify_follows_tag_push(self):
    ...
    def test_release_cut_poll_loop_emits_progress(self):
    ...
    def test_release_cut_preserves_existing_steps(self):
    ...
    def test_release_cut_uses_submake(self):
