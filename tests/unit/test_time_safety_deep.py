...
class TestISO8601Output:
    ...
    def test_peak_pricing_window_covers_tzaware_datetime(self, hours, expected_hour):
    ...
    def test_utc_regression_constants_are_equivalent(self):
        assert UTC is UTC
...
class TestNaiveDatetimeSafety:
    ...
    def test_datetime_now_no_args_returns_naive(self):
        d = datetime.now()
        assert d.tzinfo is None, "datetime.now() without tz= is naive"

    def test_utcnow_returns_tzaware(self):
        d = _utcnow()
        assert d.tzinfo is not None
        assert d.tzinfo is UTC

    def test_datetime_utcnow_is_deprecated(self):
    ...
    def test_replace_tzinfo_does_not_convert(self):
...
class TestSafeDatetime:
    ...
    def test_safe_datetime_rejects_none(self):
        assert _safe_datetime(None) is None

    def test_safe_datetime_preserves_tzaware(self):
        aware = datetime(2026, 1, 1, tzinfo=UTC)
        assert _safe_datetime(aware) is aware

    def test_safe_datetime_assumes_utc_for_naive(self):
    ...
    def test_safe_datetime_non_datetime_returns_none(self):
...
class TestMonotonicForDurations:
    ...
    def test_perf_counter_monotonic_too(self):
        """perf_counter is also monotonic (highest resolution)."""
        a = time.perf_counter()
        b = time.perf_counter()
        assert b >= a

    def test_monotonic_is_not_affected_by_system_clock(self):
    ...
    def test_wall_clock_can_jump(self):
        """time.time() can jump backwards — do NOT use for durations."""
        t1 = time.time()
        t2 = time.time()
        assert t1 >= t2 or t2 >= t1

    def test_smoke_duration_uses_monotonic(self):
    ...
    def test_cache_ttl_uses_monotonic(self):
    ...
    def test_monotonic_vs_time_divergence_detected(self):
...
class TestDSTTransitionSafety:
    ...
    def test_utc_adds_same_duration_regardless_of_dst(self):
    ...
    def test_naive_datetime_at_dst_fall_back_is_ambiguous(self):
    ...
    def test_midnight_utc_is_always_midnight(self):
        for month in range(1, 13):
            d = datetime(2026, month, 15, tzinfo=UTC)
            assert d.hour == 0
            assert d.tzinfo is UTC

    def test_wall_clock_epoch_constant(self):
...
class TestTimestampParsing:
    ...
    def test_parse_iso8601_variants(self, raw, expect):
    ...
    def test_parse_handles_log_timestamp_patterns(self):
    ...
    def test_datetime_fromisoformat_roundtrip_with_z(self):
    ...
    def test_parsed_timestamp_is_comparable_to_utc_now(self):
    ...
    @pytest.mark.parametrize(
        "fmt,value,expect",
        [
            ("%Y-%m-%dT%H:%M:%S", "2026-07-15T14:30:45", None),
            ("%Y-%m-%d", "2026-07-15", None),
            ("%Y-%m-%dT%H:%M:%S%z", "2026-07-15T14:30:45+0000", None),
            ("%Y-%m-%dT%H:%M:%S%z", "2026-07-15T14:30:45+00:00", ValueError),
            ("%Y-%m-%dT%H:%M:%S.%f", "2026-07-15T14:30:45.123456", None),
        ],
    )
    def test_strptime_behavior_with_various_formats(self, fmt, value, expect):
...
class TestComparisonSafety:
    ...
    def test_aware_vs_naive_raises_typeerror(self):
        aware = datetime.now(UTC)
        naive = datetime(2026, 1, 1)
        with pytest.raises(TypeError):
            _ = aware > naive

    def test_aware_vs_aware_tz_compare_correctly(self):
    ...
    def test_timedelta_crossing_dst_stays_consistent_in_utc(self):
...
class TestFrozenTimestamps:
    ...
    def test_utcnow_returns_mocked_time(self):
    ...
    def test_peak_pricing_now_respects_di_clock(self):
...
class TestEdgeCases:
    ...
    def test_leap_year_feb_29_safe(self):
    ...
    def test_non_leap_year_feb_28_safe(self):
        d = datetime(2025, 2, 28, tzinfo=UTC)
        next_day = d + timedelta(days=1)
        assert next_day.month == 3
        assert next_day.day == 1

    def test_timestamp_boundary_year_2038(self):
    ...
    def test_timestamp_boundary_year_2038_plus_one(self):
    ...
    def test_microsecond_precision_retained(self):
        d = datetime(2026, 7, 15, 12, 0, 0, 123456, tzinfo=UTC)
        s = d.isoformat()
        assert "123456" in s

    def test_strftime_utc_produces_expected_string(self):
