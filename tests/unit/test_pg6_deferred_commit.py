...
# ---------------------------------------------------------------------------
# 10. defer_commit failure-path: commit exception is logged at ERROR, not swallowed
# ---------------------------------------------------------------------------
def test_defer_commit_logs_error_on_commit_failure(caplog: pytest.LogCaptureFixture) -> None:
    """A deferred commit that raises must log at ERROR, not silently swallow."""
    import logging

    async def _run() -> None:
        engine = ExecutionEngine(workspace_path="/tmp/test-pg6-fail")

        async def _failing_commit(path: str, msg: str) -> str | None:
            raise RuntimeError("simulated commit failure")

        with (
            patch(
                "general_ludd.execution.engine._git_commit_async",
                side_effect=_failing_commit,
            ),
            caplog.at_level(logging.ERROR, logger="general_ludd.execution.engine"),
        ):
            engine.defer_commit("/tmp/test-pg6-fail", "failing commit msg")
            # Allow the background task to run and the callback to fire
            await asyncio.sleep(0.05)

        # The error must have been logged — NOT silently swallowed
        error_records = [r for r in caplog.records if r.levelno >= logging.ERROR]
        assert error_records, (
            "Expected an ERROR log from defer_commit on commit failure, got none. "
            f"All records: {[(r.levelname, r.message) for r in caplog.records]}"
        )
        assert any(
            "commit" in r.message.lower() or "simulated" in r.message.lower()
            for r in error_records
        ), f"Error log did not mention commit failure: {[r.message for r in error_records]}"
