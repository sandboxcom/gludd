...
def test_normalize_reopens_checked_item_without_measurable_evidence(tmp_path, monkeypatch):
    module = _module()
...
def test_normalize_preserves_prior_wave_evidence_as_pending(tmp_path, monkeypatch):
    module = _module()
    tasks = tmp_path / "TASKS.md"
...
def test_normalize_canonicalizes_fields_and_duplicate_ids(tmp_path, monkeypatch):
    module = _module()
    tasks = tmp_path / "TASKS.md"
    tasks.write_text(
