...
class TestBinaryPathResolver:
    ...
    def test_resolve_falls_back_to_which(self):
        bp = BinaryPaths()
        resolver = BinaryPathResolver(config=bp)
        with patch("agentic_harness.config.binary_paths.shutil.which", return_value="/usr/bin/git"):
            assert resolver.resolve("git") == "/usr/bin/git"

    def test_resolve_returns_name_when_not_found(self):
        bp = BinaryPaths()
        resolver = BinaryPathResolver(config=bp)
        with patch("agentic_harness.config.binary_paths.shutil.which", return_value=None):
    ...
    def test_get_infra_binary_prefers_tofu(self):
        bp = BinaryPaths()
        resolver = BinaryPathResolver(config=bp)
        with patch("agentic_harness.config.binary_paths.shutil.which", return_value="/usr/bin/tofu"):
    ...
    def test_get_infra_binary_falls_back_to_terraform(self):
        ...
        with patch("agentic_harness.config.binary_paths.shutil.which", side_effect=which_fn):
            assert resolver.get_infra_binary() == "terraform"

    def test_get_secrets_binary_prefers_bao(self):
        bp = BinaryPaths()
        resolver = BinaryPathResolver(config=bp)
        with patch("agentic_harness.config.binary_paths.shutil.which", return_value="/usr/bin/bao"):
    ...
    def test_get_container_runtime_prefers_podman(self):
        bp = BinaryPaths()
        resolver = BinaryPathResolver(config=bp)
        with patch("agentic_harness.config.binary_paths.shutil.which", return_value="/usr/bin/podman"):
