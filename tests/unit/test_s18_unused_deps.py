"""S.18 — Verify provider/model dependencies match runtime usage."""
import re
import tomllib
...
def test_langchain_openai_kept_for_provider_gateway():
    deps = _load_deps()
    assert "langchain-openai" in deps, "langchain-openai powers OpenAI-compatible provider presets"
    provider_registry = (SRC_DIR / "general_ludd" / "models" / "provider_registry.py").read_text()
    provider_presets = (SRC_DIR / "general_ludd" / "models" / "provider_presets.py").read_text()
    assert "langchain_openai" in provider_registry
    assert "langchain-openai" in provider_presets
