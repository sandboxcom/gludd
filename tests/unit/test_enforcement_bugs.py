...
class TestMultitaskDeadFloorBreach:
    ...
    def test_zero_streak_reset_on_dispatch_is_correct(self):
        ...
        body = src[start:end]
        prev = body.index("s.prevMessageDispatches = s.thisMessageDispatches")
        zero_branch = body.index("if (s.thisMessageDispatches === 0)")
        increment = body.index("s.zeroStreak++", zero_branch)
        reset = body.index("s.zeroStreak = 0", increment)
        assert prev < zero_branch < increment < reset, (
            "handleMessageBoundary must record the wave, increment zeroStreak "
            "for an empty wave, and reset it after a dispatch wave"
        )
...
class TestBugsMDOpenWorkDetection:
    ...
    def test_incident_filter_checks_entire_incident_section(self):
        """FIX VERIFICATION: resolution markers are read from the full section."""
        src = _src(FLOOR_PATH)
        start = src.index("function countOpenBugIncidents")
        end = src.index("function openWorkExists", start)
        body = src[start:end]
        assert "incidentSections" in body
        assert 'current.join("\\n")' in body
        assert ".filter(incident =>" in body
        assert ".test(incident)" in body
        assert "countOpenBugIncidents(fs.readFileSync(bugsMd" in src
...
class TestGitIndexMtimeFalsePositive:
    """Git index mtime comparison was replaced with porcelain status."""

    def test_index_mtime_comparison_removed(self):
        """FIX VERIFICATION: pending work uses git status, never mtimes."""
        src = _src(FLOOR_PATH)
...
class TestDispatchTypo:
    """Console.warn message spells DISPATCH correctly."""

    def test_dispatch_spelling_in_refill_warning(self):
        """FIX VERIFICATION: the historical DISPTACH typo stays removed."""
        src = _src(FLOOR_PATH)
