...
class TestZaiSecretsResolution:
    ...
    def test_zai_api_key_uppercase_env_resolves(self):
        ...
        os.environ["ZAI_API_KEY"] = "test-key-uppercase"  # pragma: allowlist secret
        mgr = EnvSecretsManager()
        assert mgr.resolve("zai_api_key") == "test-key-uppercase"  # pragma: allowlist secret
    ...
    def test_lowercase_alias_direct_env_still_works(self):
        ...
        os.environ["ZAI_API_KEY"] = "direct-key-lowercase"  # pragma: allowlist secret
        mgr = EnvSecretsManager()
        assert mgr.resolve("ZAI_API_KEY") == "direct-key-lowercase"  # pragma: allowlist secret

    def test_override_takes_precedence_over_env(self):
        """Explicit set() always wins over ambient env."""
        os.environ["ZAI_API_KEY"] = "env-key"  # pragma: allowlist secret
        mgr = EnvSecretsManager()
        mgr.set("zai_api_key", "override-key")  # pragma: allowlist secret
        assert mgr.resolve("zai_api_key") == "override-key"  # pragma: allowlist secret

    def test_non_allowlisted_name_still_blocked(self):
        """Security regression: non-allowlisted names are still blocked."""
        os.environ["GLUDD_PSK"] = "should-not-resolve"  # pragma: allowlist secret
        mgr = EnvSecretsManager()
