    ...
    return FlagEvaluator(
        [
            ...
            FeatureFlag(
                ...
                default=False,
                targeting_rules=[
                    TargetingRule("role", "admin", operator="eq"),
                ],
            ),
            FeatureFlag("beta_search", default=False, dependencies=["dark_mode"]),
            FeatureFlag(
                "advanced_analytics", default=False, dependencies=["beta_search", "admin_panel"], depends_on_all=True
            ),
            FeatureFlag("export_csv", default=False, dependencies=["dark_mode", "enable_new_ui"], depends_on_all=False),
        ]
    )
...
class TestTargetingRules:
    ...
    def test_targeting_rule_takes_priority_over_default(self, evaluator: FlagEvaluator) -> None:
        evaluator.register(
            FeatureFlag(
                "legacy",
                default=True,
        ...
        assert not evaluator.is_enabled("legacy", {"tenant": "new_cloud"})
...
class TestFlagDependencyChains:
    ...
    def test_disabled_dependency_blocks(self, evaluator: FlagEvaluator) -> None:
        evaluator.register(
            ...
            FeatureFlag("child", default=True, dependencies=["parent"], depends_on_all=True),
        )
    ...
    def test_transitive_dependency_block(self, evaluator: FlagEvaluator) -> None:
        evaluator.register(
            ...
            FeatureFlag("b", default=True, dependencies=["a"]),
            FeatureFlag("c", default=True, dependencies=["b"], depends_on_all=True),
        )
