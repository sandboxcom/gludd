...
def test_aws_ec2_a100_accepts_user_friendly_env_aliases_and_redacts_values() -> None:
    ...
    assert "redacted-value" not in encoded
...
def test_live_metadata_probe_records_http_metrics_without_payload_dump() -> None:
    ...
    assert "credential-value" not in json.dumps(report)
...
def test_live_metadata_auth_rejection_is_not_provider_health_pass() -> None:
    ...
    report = run_smoke(
        "openrouter",
        "metadata",
        env={"OPENROUTER_API_KEY": "fake-credential"},
        live=True,
        http_get=fake_http_get,
    )
...
def test_smoke_reports_trace_sequence_and_declared_coverage_depth() -> None:
    report = run_smoke(
        "aws",
        "ec2-a100",
        env={"AWS_KEY": "AKIA_TEST_VALUE", "AWS_SECRET": "redacted-value"},
    )
...
def test_live_model_ping_posts_minimal_request_and_requires_completion(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OPENROUTER_API_KEY", "real-looking-credential")
    ...
    report = run_smoke(
        "openrouter",
        "model-ping",
        env={"OPENROUTER_API_KEY": "real-looking-credential"},
        live=True,
...
def test_cli_smoke_json_writes_full_diagnostic_bundle(
    ...
    monkeypatch.setenv("AWS_SECRET", "redacted-value")
...
def test_multi_provider_model_juggle_live_posts_configured_provider_legs() -> None:
    ...
    report = run_smoke(
        "multi-provider",
        "model-juggle",
        env={"OPENROUTER_API_KEY": "openrouter-credential", "GROQ_API_KEY": "groq-credential"},
        live=True,
        http_post=fake_http_post,
    )
    ...
    assert "openrouter-credential" not in encoded
    assert "groq-credential" not in encoded
