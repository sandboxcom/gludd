"""Tests for ``gludd test-bg`` CLI subcommand.

Covers parser registration / handler wiring plus each subcommand's behavior
against a mock ``BackgroundTestRunner``.
"""
...
def _testbg_subactions() -> argparse._SubParsersAction[argparse.ArgumentParser]:
    """Return the nested subparsers action for ``gludd test-bg``."""
    _parser, subcommand_map = cli_mod.build_parser()
    assert "test-bg" in subcommand_map, "test-bg not registered in top-level subcommand_map"
    test_bg_parser = subcommand_map["test-bg"]
    for action in test_bg_parser._subparsers._group_actions:  # type: ignore[union-attr]
        if isinstance(action, argparse._SubParsersAction):
            return action
    raise AssertionError("test-bg parser has no nested subparsers action")
...
class TestParserRegistration:
    def test_top_level_command_registered(self) -> None:
        _parser, subcommand_map = cli_mod.build_parser()
        assert "test-bg" in subcommand_map

    def test_all_five_subcommands_registered(self) -> None:
        sub_action = _testbg_subactions()
...
class TestArgumentParsing:
    """Verify each subparser accepts its expected arguments."""
    ...
    def test_poll_all_accepts_no_args(self) -> None:
        _parser, subcommand_map = cli_mod.build_parser()
        test_bg = subcommand_map["test-bg"]
        ns = test_bg.parse_args(["poll-all"])
