...
class TestExtractMediaQueries:
    def test_min_width(self):
        ...
        assert any("min-width" in q for q in queries)

    def test_max_width(self):
        css = "@media (max-width: 600px) { body { margin: 0; } }"
        queries = extract_media_queries(css)
        assert any("max-width" in q for q in queries)

    def test_combined_query(self):
        css = "@media (min-width: 768px) and (max-width: 1024px) { body { margin: 0; } }"
        queries = extract_media_queries(css)
        assert any("min-width" in q and "max-width" in q for q in queries)
...
class TestCheckResponsivePatterns:
    def test_flexbox_found(self):
        ...
        assert any("flex" in p.lower() for p in patterns)

    def test_grid_found(self):
        css = ".layout { display: grid; grid-template-columns: 1fr 1fr; }"
        patterns = check_responsive_patterns(css)
        assert any("grid" in p.lower() for p in patterns)

    def test_neither_found(self):
        css = "body { display: block; }"
        patterns = check_responsive_patterns(css)
        assert not any("flex" in p.lower() for p in patterns)
        assert not any("grid" in p.lower() for p in patterns)


class TestGenerateBoilerplate:
    def test_html5_type(self):
        html = generate_boilerplate(doc_type="html5")
...
class TestExtractColorsFromCss:
    ...
    def test_rgb_colors(self):
        ...
        colors = extract_colors_from_css(css)
        assert any("255" in c for c in colors)
...
class TestExtractFontsFromCss:
    ...
    def test_google_fonts_import(self):
        ...
        fonts = extract_fonts_from_css(css)
        assert any("Roboto" in f for f in fonts)
...
class TestDetectCssFramework:
    def test_tailwind_classes(self):
        html = "<div class=\"flex items-center justify-between p-4\"></div>"
        framework = detect_css_framework(html)
        assert framework is not None
        assert "tailwind" in framework.lower()

    def test_bootstrap_classes(self):
        html = "<div class=\"container mt-3 row\"><div class=\"col-md-6\"></div></div>"
        framework = detect_css_framework(html)
        assert framework is not None
...
class TestCheckJsSyntax:
    ...
    def test_unclosed_brace(self):
        ...
        errors = check_js_syntax(js)
        assert any("unclosed" in e.lower() or "brace" in e.lower() or "{" in e for e in errors)
...
class TestDetectErrorPatterns:
    def test_console_log_left_in(self):
        ...
        assert any("console.log" in p.lower() for p in patterns)

    def test_eqeq_instead_of_eqeqeq(self):
        js = "function check(x) { if (x == null) return true; }"
        patterns = detect_error_patterns(js)
        assert any("==" in p or "eqeq" in p.lower() for p in patterns)

    def test_var_instead_of_let(self):
        js = "var count = 0;"
        patterns = detect_error_patterns(js)
        assert any("var" in p.lower() for p in patterns)
...
class TestVerifySourceMap:
    def test_valid_map_format(self):
        ...
        assert verify_source_map(content) is True

    def test_invalid_json(self):
        content = "not a source map"
        assert verify_source_map(content) is False
...
class TestGenerateReactComponent:
    ...
    def test_typescript_variant(self):
        code = generate_react_component("Card", props=["title"], typescript=True)
        assert "Card" in code
...
class TestGenerateHtmxTemplate:
    def test_get_trigger(self):
        html = generate_htmx_template("click", method="get")
        assert "hx-get" in html or "hx-trigger" in html

    def test_post_trigger_with_target(self):
        html = generate_htmx_template("submit", method="post", target="#result")
...
class TestGenerateGraphqlQuery:
    ...
    def test_with_variables(self):
        query = generate_graphql_query("GetPost", ["title", "body"], variables={"id": "ID!"})
...
class TestGenerateSpacingTokens:
    def test_4px_base(self):
        tokens = generate_spacing_tokens(base_px=4, steps=6)
        assert "spacing-0" in tokens or "0" in str(tokens)
        assert len(tokens) >= 6

    def test_8px_base(self):
        tokens = generate_spacing_tokens(base_px=8, steps=6)
        assert len(tokens) >= 6

    def test_custom_steps(self):
        tokens = generate_spacing_tokens(base_px=4, steps=4)
        assert len(tokens) == 4


class TestGenerateColorTokens:
    def test_single_color_to_shades(self):
        tokens = generate_color_tokens("#3366cc", num_shades=5)
        assert len(tokens) >= 5


class TestGenerateTypeScale:
    def test_1_25_ratio(self):
        scale = generate_type_scale(ratio=1.25, base_size=16, num_steps=6)
        assert len(scale) >= 6

    def test_1_5_ratio(self):
        scale = generate_type_scale(ratio=1.5, base_size=16, num_steps=6)
        assert len(scale) >= 6
