...
def _run_grind_test_harness(
    ...
    (tmp_path / "TASKS.md").write_text("- [ ] Grind test pending work item\n")
...
class TestAntiGrindStructural:
    ...
    def test_streak_count_incremented_for_non_dispatch(self):
        src = self._src()
        assert "_streakCount++" in src, "_streakCount MUST be incremented for non-dispatch calls"

    def test_read_tools_do_not_increment_streak_count(self):
        src = self._src()
    ...
    def test_dispatch_resets_streak_count_to_zero(self):
        src = self._src()
    ...
    def test_floor_breach_gated_on_streak_gt_max(self):
        src = self._src()
    ...
    def test_block_message_includes_streak_count_and_floor(self):
        src = self._src()
    ...
    def test_read_grinding_block_at_15_reads_60_seconds(self):
        src = self._src()
    ...
    def test_compulsive_check_commands_are_blocked(self):
        src = self._src()
        assert "COMPULSIVE_CHECK_RE" in src

    def test_session_start_read_grinding_block(self):
        src = self._src()
...
class TestAntiGrindRuntime:
    ...
    def test_first_non_dispatch_call_is_allowed(self, tmp_path: Path):
    ...
    def test_four_consecutive_non_dispatch_calls_triggers_block(self, tmp_path: Path):
    ...
    def test_dispatch_resets_streak(self, tmp_path: Path):
    ...
    def test_read_only_tools_do_not_increment_streak(self, tmp_path: Path):
    ...
    def test_mixed_read_then_edit_triggers_block(self, tmp_path: Path):
    ...
    def test_single_dispatch_resets_everything(self, tmp_path: Path):
    ...
    def test_env_var_disables_all_floor_enforcement(self, tmp_path: Path):
