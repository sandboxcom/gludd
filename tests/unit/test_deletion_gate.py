...
def test_edit_with_deletion_reason_allowed():
    ...
    os.environ["DELETION_REASON"] = "Refactoring old feature X"
    reason = os.environ.get("DELETION_REASON")
    assert reason == "Refactoring old feature X"
    del os.environ["DELETION_REASON"]


def test_write_with_deletion_reason_allowed():
    ...
    os.environ["DELETION_REASON"] = "Removing deprecated module"
    reason = os.environ.get("DELETION_REASON")
...
def test_threshold_configurable_via_env():
    ...
    # Default threshold
    os.environ.pop("GLUDD_DELETION_GATE_THRESHOLD", None)
    threshold = int(os.environ.get("GLUDD_DELETION_GATE_THRESHOLD", "5"))
    assert threshold == 5

    # Custom threshold
    os.environ["GLUDD_DELETION_GATE_THRESHOLD"] = "10"
    threshold = int(os.environ.get("GLUDD_DELETION_GATE_THRESHOLD", "5"))
    assert threshold == 10
    del os.environ["GLUDD_DELETION_GATE_THRESHOLD"]
...
def test_zero_threshold_allows_all_deletions():
    """Test that threshold of 0 allows all deletions (no gate)."""
    os.environ["GLUDD_DELETION_GATE_THRESHOLD"] = "0"
    threshold = int(os.environ.get("GLUDD_DELETION_GATE_THRESHOLD", "5"))
    assert threshold == 0
