...
class TestBatchPush:
    ...
    def test_blocks_threshold_one_override(self):
        recipe = _recipe("batch-push")
...
class TestShipCommit:
    ...
    def test_push_default_is_zero(self):
        content = _content()
        assert "PUSH ?= 0" in content, (
            "ship-commit must default PUSH=0 (no auto-push on commit)"
        )

    def test_does_not_force_push_by_default(self):
...
class TestReleaseCut:
    """MK.10 / RL.2 / EX.1 — release-cut pipeline gates on require-ci-green."""

    def test_target_exists(self):
        assert _recipe("release-cut"), "release-cut target must exist (MK.10)"

    def test_calls_require_ci_green(self):
        recipe = _recipe("release-cut")
        assert "require-ci-green" in recipe, (
            "release-cut must invoke require-ci-green as step 0 (RL.2)"
        )

    def test_calls_check_readme_status(self):
        recipe = _recipe("release-cut")
...
class TestReleaseTagPush:
    ...
    def test_cancels_master_ci(self):
    ...
    def test_pushes_master_and_tag(self):
        recipe = _recipe("release-tag-push")
...
class TestCiCancel:
    ...
    def test_accepts_RUN_argument(self):
...
class TestPushRateGuard:
    ...
    def test_checks_ci_in_flight(self):
...
class TestNoCommitThresholdOneDefault:
    ...
    def test_no_commit_threshold_default_assignment_to_one(self):
    ...
    def test_batch_push_does_not_default_to_one(self):
        recipe = _recipe("batch-push")
...
def test_target_present_in_makefile(target):
