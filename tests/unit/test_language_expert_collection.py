...
class TestUnicodeData:
    ...
    def test_module_importable(self) -> None:
        from src.general_ludd.language import unicode_data
        assert unicode_data is not None

    def test_unicode_planes_all_defined(self) -> None:
        from src.general_ludd.language.unicode_data import UNICODE_PLANE_NAMES, UnicodePlane
    ...
    def test_unicode_version_history_complete(self) -> None:
        from src.general_ludd.language.unicode_data import UNICODE_VERSION_HISTORY
    ...
    def test_unicode_categories_all_30(self) -> None:
        from src.general_ludd.language.unicode_data import UNICODE_CATEGORY_NAMES
        assert len(UNICODE_CATEGORY_NAMES) == 30

    def test_unicode_blocks_cover_basic_ranges(self) -> None:
        from src.general_ludd.language.unicode_data import UNICODE_BLOCK_NAMES
    ...
    def test_surrogate_functions(self) -> None:
        from src.general_ludd.language.unicode_data import (
            is_high_surrogate,
            is_low_surrogate,
            is_surrogate,
    ...
    def test_plane_of(self) -> None:
        from src.general_ludd.language.unicode_data import plane_of
    ...
    def test_utf8_header_bytes_valid(self) -> None:
        from src.general_ludd.language.unicode_data import UTF8_HEADER_BYTES
...
class TestUnicodeDataBehavioral:
    ...
    def test_surrogate_high_low_boundaries(self) -> None:
        from src.general_ludd.language.unicode_data import is_high_surrogate, is_low_surrogate, is_surrogate
        assert is_high_surrogate(0xD800)
    ...
    def test_surrogates_to_codepoint_min_supplementary(self) -> None:
        from src.general_ludd.language.unicode_data import surrogates_to_codepoint
...
class TestCharsetMap:
    ...
    def test_module_importable(self) -> None:
        from src.general_ludd.language import charset_map
        assert charset_map is not None

    def test_bom_signatures_all_five(self) -> None:
        from src.general_ludd.language.charset_map import BOM_SIGNATURES
    ...
    def test_bom_reverse_lookup(self) -> None:
        from src.general_ludd.language.charset_map import BOM_BY_SEQUENCE
    ...
    def test_bom_required_optional(self) -> None:
        from src.general_ludd.language.charset_map import (
            BOM_OPTIONAL_BY_RFC,
            BOM_REQUIRED_BY_RFC,
        )
    ...
    def test_iso_8859_all_covered(self) -> None:
        from src.general_ludd.language.charset_map import SINGLE_BYTE_ENCODINGS
    ...
    def test_windows_code_pages_all(self) -> None:
        from src.general_ludd.language.charset_map import WINDOWS_CODE_PAGES
    ...
    def test_cjk_encodings_all_six(self) -> None:
        from src.general_ludd.language.charset_map import CJK_ENCODINGS
    ...
    def test_cyrillic_encodings(self) -> None:
        from src.general_ludd.language.charset_map import CYRILLIC_ENCODINGS
    ...
    def test_ibm_code_pages_all_13(self) -> None:
        from src.general_ludd.language.charset_map import IBM_CODE_PAGES
        assert len(IBM_CODE_PAGES) == 13

    def test_mojibake_signatures_have_entries(self) -> None:
        from src.general_ludd.language.charset_map import MOJIBAKE_SIGNATURES
...
class TestCharsetMapBehavioral:
    ...
    def test_detect_bom_utf8(self) -> None:
        from src.general_ludd.language.charset_map import BOM_BY_SEQUENCE, BOM_SIGNATURES
    ...
    def test_bom_size_lookup(self) -> None:
        from src.general_ludd.language.charset_map import BOM_SIZE
    ...
    def test_chardet_thresholds_ordered(self) -> None:
        from src.general_ludd.language.charset_map import CHARDET_CONFIDENCE_THRESHOLDS
...
class TestLocaleData:
    ...
    def test_locale_formats_defined(self) -> None:
        from src.general_ludd.language.locale_data import LOCALE_FORMATS
    ...
    def test_iso_639_1_complete(self) -> None:
        from src.general_ludd.language.locale_data import ISO_639_1_TO_NAME
...
class TestLocaleDataBehavioral:
    ...
    def test_rtl_locales_have_rtl_scripts(self) -> None:
        from src.general_ludd.language.locale_data import LOCALE_FORMATS, RTL_SCRIPTS
        for key, locale in LOCALE_FORMATS.items():
            if locale["is_rtl"]:
    ...
    def test_locale_territories_in_iso_3166(self) -> None:
        from src.general_ludd.language.locale_data import ISO_3166_TO_NAME, LOCALE_FORMATS
        for key, locale in LOCALE_FORMATS.items():
    ...
    def test_locale_scripts_in_iso_15924(self) -> None:
        from src.general_ludd.language.locale_data import ISO_15924_TO_NAME, LOCALE_FORMATS
        for key, locale in LOCALE_FORMATS.items():
...
class TestPhoneticData:
    ...
    def test_module_importable(self) -> None:
        from src.general_ludd.language import phonetic_data
        assert phonetic_data is not None

    def test_ipa_vowels_defined(self) -> None:
        from src.general_ludd.language.phonetic_data import IPA_VOWELS
        assert len(IPA_VOWELS) >= 20

    def test_ipa_consonants_defined(self) -> None:
        from src.general_ludd.language.phonetic_data import IPA_CONSONANTS
        assert len(IPA_CONSONANTS) >= 20

    def test_arpabet_to_ipa_complete(self) -> None:
        from src.general_ludd.language.phonetic_data import ARPABET_TO_IPA
        assert len(ARPABET_TO_IPA) >= 35

    def test_ipa_to_arpabet_reverse(self) -> None:
        from src.general_ludd.language.phonetic_data import IPA_TO_ARPABET
        assert len(IPA_TO_ARPABET) >= 30

    def test_soundex_mapping_complete(self) -> None:
        from src.general_ludd.language.phonetic_data import SOUNDEX_MAPPING
        assert len(SOUNDEX_MAPPING) >= 18
        assert SOUNDEX_MAPPING["b"] == "1"

    def test_cmu_dict_subset(self) -> None:
        from src.general_ludd.language.phonetic_data import CMU_DICT_SUBSET
...
class TestPhoneticDataBehavioral:
    ...
    def test_arpabet_ipa_bidirectional_consistency(self) -> None:
        from src.general_ludd.language.phonetic_data import ARPABET_TO_IPA, IPA_TO_ARPABET
        for arp, ipa in ARPABET_TO_IPA.items():
    ...
    def test_soundex_vowels_no_mapping(self) -> None:
        from src.general_ludd.language.phonetic_data import SOUNDEX_MAPPING, SOUNDEX_VOWELS
    ...
    def test_soundex_ignore_no_mapping(self) -> None:
        from src.general_ludd.language.phonetic_data import SOUNDEX_IGNORE, SOUNDEX_MAPPING
    ...
    def test_arpabet_stress_three_levels(self) -> None:
        from src.general_ludd.language.phonetic_data import ARPABET_STRESS
    ...
    def test_metaphone_exceptions_are_pairs(self) -> None:
        from src.general_ludd.language.phonetic_data import METAPHONE_EXCEPTIONS
        for key, val in METAPHONE_EXCEPTIONS.items():
            assert len(key) == 2
            assert len(val) == 1

    def test_double_metaphone_values_are_pairs(self) -> None:
        from src.general_ludd.language.phonetic_data import DOUBLE_METAPHONE
...
class TestHomoglyphData:
    ...
    def test_module_importable(self) -> None:
        from src.general_ludd.language import homoglyph_data
        assert homoglyph_data is not None

    def test_homoglyph_groups_defined(self) -> None:
        from src.general_ludd.language.homoglyph_data import HOMOGLYPH_GROUPS
        assert len(HOMOGLYPH_GROUPS) >= 15

    def test_invisible_characters_defined(self) -> None:
        from src.general_ludd.language.homoglyph_data import INVISIBLE_CHARACTERS
        assert len(INVISIBLE_CHARACTERS) >= 15

    def test_attack_vectors_defined(self) -> None:
        from src.general_ludd.language.homoglyph_data import ATTACK_VECTORS
    ...
    def test_invisible_codepoints_helper(self) -> None:
        from src.general_ludd.language.homoglyph_data import _INVISIBLE_SET
...
class TestWorkingGroupKnowledge:
    ...
    def test_locale_data_references_clr(self) -> None:
        from src.general_ludd.language.locale_data import (
