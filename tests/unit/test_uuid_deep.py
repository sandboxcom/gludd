...
class TestUUID7:
    def test_generates_valid_uuid7(self):
        u = uuid7()
        assert isinstance(u, _uuid.UUID)
    ...
    def test_monotonicity_within_same_millisecond(self):
        now_ms = int(time.time() * 1000)
        ids = [uuid7(timestamp_ms=now_ms) for _ in range(100)]
        for i in range(len(ids) - 1):
            assert ids[i].int <= ids[i + 1].int

    def test_time_ordering_across_milliseconds(self):
    ...
    def test_extract_timestamp_roundtrips(self):
        now_ms = int(time.time() * 1000)
    ...
    def test_extract_timestamp_v4_returns_none(self):
        u = uuid4()
        assert extract_timestamp(u) is None


class TestUUID8:
    def test_generates_valid_uuid8(self):
...
class TestULID:
    ...
    def test_crockford_alphabet_only(self):
        uid = ulid()
    ...
    def test_parse_ulid_invalid_character(self):
        with pytest.raises(ValueError, match="Invalid ULID character"):
            parse_ulid("ILLEGALULIDCHARS!!!!!!!")

    def test_extract_ulid_timestamp(self):
        now_ms = int(time.time() * 1000)
        uid = ulid(timestamp_ms=now_ms)
        ts = extract_ulid_timestamp(uid)
        assert ts == now_ms

    def test_extract_ulid_timestamp_invalid(self):
...
class TestValidation:
    ...
    def test_is_valid_uuid_v7(self):
        u = str(uuid7())
        assert is_valid_uuid(u, version=7)

    def test_is_valid_uuid_v8(self):
        u = str(uuid8(b"aa" * 3, b"bb" * 2, b"cc" * 3))
        assert is_valid_uuid(u, version=8)

    def test_is_valid_uuid_wrong_version(self):
        u = str(uuid4())
    ...
    def test_is_valid_ulid_true(self):
        uid = ulid()
        assert is_valid_ulid(uid)

    def test_is_valid_ulid_false(self):
