"""Tests for HarnessRunner — runs generated pytest files and captures output."""
...
class HarnessRunnerCreation:
    ...
    def test_creates_with_custom_args(self) -> None:
        runner = HarnessRunner(pytest_args=["-v", "-x"], timeout_seconds=120)
        assert runner.pytest_args == ["-v", "-x"]
        assert runner.timeout_seconds == 120


class HarnessRunnerExecute:
    ...
    def test_timeout_kills_long_running_test(self, tmp_path: Path) -> None:
        ...
        runner = HarnessRunner(timeout_seconds=2)
        result = runner.execute(test_dir=str(tmp_path))
        assert result.returncode != 0
