...
class TestToolUseWorkTypesExpanded:
    ...
    def test_specific_code_types_exist(self):
        ...
        assert expected == _CODE_WORK_TYPES
...
class TestWorkTypeIterationCaps:
    ...
    async def test_code_work_type_uses_code_max_iterations(self):
        ...
        gateway = MagicMock()
        gateway.call_model = MagicMock(return_value="done")
        ...
        # No mcp_client → falls through to plain _call_model.
        loop = ToolCallLoop(
    ...
    async def test_code_work_type_iteration_limit_raises(self):
        ...
        with patch.object(
            ToolCallLoop, "_resolve_server_id", return_value="echo",
        ), patch.object(
            ToolCallLoop, "_call_with_tools", new_callable=AsyncMock,
        ) as mock_ct:
            mock_ct.return_value = resp
            loop = ToolCallLoop(
                model_gateway=gateway,
                mcp_client=mcp,
                max_iterations=10,
