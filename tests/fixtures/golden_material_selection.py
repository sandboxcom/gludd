...
GOLDEN_PROBLEMS: list[GoldenSelectionProblem] = [
    ...
    GoldenSelectionProblem(
        ...
        description="Select material for a structural I-beam. "
        "Key factors: yield strength >250 MPa, modulus for deflection control, cost.",
        requirements={
        ...
        ranking_rationale="al7075_t6 has best specific strength "
        "(yield/density = 179 vs 66 vs 102), lowest weight for same capacity",
    ),
    # ── 2. Pressure vessel (alloy steel, safety-critical) ───────────────────
    GoldenSelectionProblem(
        id="MATE-GS-002",
        title="Pressure vessel — chemical reactor shell, safety-critical",
        description="ASME BPVC Section VIII Div 1 vessel. "
        "Requires yield >200 MPa, fracture toughness, and weldability. Safety-critical.",
        requirements={
        ...
        ranking_rationale="Per pure yield margin (310 vs 290 vs 215 for 200 MPa): 1045 ranks #1. "
        "Note: corrosion is a tradeoff not captured in load-case-only ranking; "
        "316L would win if environment scoring were active",
    ),
    # ── 3. Consumer electronics enclosure (polymer, light loads) ────────────
    GoldenSelectionProblem(
        id="MATE-GS-003",
        title="Laptop enclosure — thin-walled plastic shell",
        description="Consumer electronics housing. "
        "Key: low weight, impact resistance, cost, flame rating (UL94 V-0), tensile >30 MPa.",
        requirements={
        ...
        ranking_rationale="Per pure margin: pa66_gf30 (180 MPa yield → 5.0) beats PC (65 MPa → 1.17). "
        "Note: overkill on strength but ranked highest by yield margin alone. "
        "Impact/flame tradeoffs not yet scored.",
    ),
    # ── 4. Aircraft bracket (aluminum, fatigue-limited) ─────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-004",
        title="Wing rib bracket — fatigue-limited aluminum part",
        description="Stiffener bracket on wing rib. Lightweight critical. "
        "Yield >150 MPa, high specific stiffness preferred.",
        requirements={
        ...
        ranking_rationale="Per pure yield margin: ti_6al4v (880 MPa → 4.87) beats al7075_t6 (503 → 2.35). "
        "Specific strength (al wins) not captured in margin-only ranking. "
        "Safety-critical triggers human review.",
    ),
    # ── 5. Automotive panel (steel, formability) ────────────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-005",
        title="Door outer panel — deep-drawn steel sheet",
        description="Automotive body panel requiring deep drawing. "
        "Yield >150 MPa, good formability for complex curvature.",
        requirements={
        ...
        expected_rejections=[],
        best_candidate="aisi_1045",
        ranking_rationale="Steel best formability and cost for automotive panels. "
        "Aluminum requires more expensive tooling; stainless overqualified cost-wise.",
    ),
    # ── 6. Medical implant (titanium, biocompatible) ────────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-006",
        title="Hip stem — osseointegration implant",
        description="Load-bearing orthopedic implant. "
        "Requires yield >500 MPa, biocompatibility, osseointegration, "
        ...
        best_candidate="ti_6al4v_annealed",
        ranking_rationale="Ti-6Al-4V is the gold standard for implants — "
        "biocompatible, osseointegrates, excellent corrosion. "
        "4340 passes yield but not biocompatible over long term.",
    ),
    # ── 7. Pipe fitting (brass vs stainless, corrosion) ─────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-007",
        title="Plumbing fitting — potable water, corrosion-resistant",
        description="Residential/commercial pipe fitting for potable water. "
        "Yield >200 MPa, corrosion resistance to chlorinated water, machinability.",
        requirements={
        ...
        ranking_rationale="Brass preferred for plumbing: self-lubricating, antimicrobial, "
        "good machinability. Stainless also passes but higher cost for potable water.",
    ),
    # ── 8. Heat shield (ceramic, high-temp) ─────────────────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-008",
        title="Furnace liner — heat shield panel",
        description="Heat shield inside industrial furnace. "
        "Must survive compressive stress 100 MPa at >800C. Metals lose strength above 500C.",
        requirements={
        ...
        ranking_rationale="Only ceramic survives due to compressive_strength property availability. "
        "Metals have no compressive_strength property in registry.",
    ),
    # ── 9. Bearing (hardened steel, wear) ───────────────────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-009",
        title="Rolling-element bearing race — high contact stress",
        description="Bearing inner race. Hertzian contact stress high, "
        "need compressive yield >800 MPa and wear resistance.",
        requirements={
        ...
        ranking_rationale="4340 has compressive_strength=860 MPa, margin=0.075. "
        "1045 lacks compressive_strength → rejected. Hardened 4340 is bearing-grade.",
    ),
    # ── 10. Spring (music-wire analog, high yield) ──────────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-010",
        title="Compression spring — high-cycle fatigue application",
        description="Helical compression spring for valve mechanism. "
        "Need yield >800 MPa, high elastic resilience (yield^2/E), fatigue resistance.",
        requirements={
        ...
        ranking_rationale="Maraging 250 has highest yield (1790 MPa) → margin 1.24. "
        "CuBe (1.38 margin) has slightly higher margin but maraging is better for fatigue.",
    ),
    ...
    GoldenSelectionProblem(
        ...
        expected_rejections=[],
        best_candidate="ti_6al4v_annealed",
        ranking_rationale="Per pure yield margin: ti_6al4v (880 → 3.40) beats aa6061_t6 (276 → 0.38) "
        "which beats az31b_h24 (220 → 0.10). "
        "Specific strength (Mg best at 124) not captured in margin-only ranking.",
    ),
    # ── 12. Chemical vessel lining (polymer, chemical resistance) ───────────
    GoldenSelectionProblem(
        id="MATE-GS-012",
        title="Acid storage tank liner — chemical resistance critical",
        description="Liner for sulfuric acid tank. "
        "Loads are minimal (tensile 20 MPa). Chemical resistance is the dominant constraint.",
        requirements={
        ...
        best_candidate="ptfe",
        ranking_rationale="PTFE is the gold standard for chemical resistance (near-universal). "
        "HDPE also resists acids but PTFE has wider temp range. "
        "Epoxy_cast rejected — has only ultimate_strength, no yield_strength.",
    ),
    # ── 13. Cutting tool (cemented carbide, extreme hardness) ───────────────
    GoldenSelectionProblem(
        id="MATE-GS-013",
        title="CNC insert — high-speed machining of hardened steel",
        description="Turning insert for hardened steel (HRC 55+). "
        "Compressive stress at cutting edge >1500 MPa. Hot hardness critical above 600C.",
        requirements={
        ...
        ranking_rationale="Both ceramics pass. WC-Co preferred for toughness — "
        "alumina is too brittle for interrupted cuts. WC-Co margin 2.67 vs alumina 0.40.",
    ),
    # ── 14. Electrical bus bar (copper, conductivity) ───────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-014",
        title="Switchgear bus bar — high conductivity",
        description="Main bus bar in low-voltage switchgear. "
        "Mechanical loads are incidental (tensile 100 MPa). "
        "Electrical conductivity is the key property.",
        requirements={
        ...
        ranking_rationale="ETP copper is the standard for electrical bus — "
        "highest conductivity (101% IACS). Brass conducts ~26% IACS. "
        "Aluminum is used but copper is standard for bus bars.",
    ),
    # ── 15. Aerospace fastener (titanium, shear + tension) ──────────────────
    GoldenSelectionProblem(
        id="MATE-GS-015",
        title="Hi-Lok fastener — shear-critical aerospace joint",
        description="Aerospace structural fastener in double-shear. "
        "Yield >600 MPa, shear strength critical for joint design.",
        requirements={
            "load_cases": [
        ...
        expected_rejections=[],
        best_candidate="ti_6al4v_annealed",
        ranking_rationale="Ti-6Al-4V is the standard aerospace fastener material — "
        "lightweight, corrosion-resistant, galvanically compatible with aluminum airframe. "
        "4340 is heavier and needs cadmium plating.",
    ),
    # ── 16. Transparent cover (acrylic vs polycarbonate) ────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-016",
        title="Instrument panel window — optical clarity required",
        description="Transparent protective window for an instrument panel. "
        "Tensile >30 MPa, optical clarity, UV resistance.",
        requirements={
        ...
        expected_rejections=[],
        best_candidate="pmma",
        ranking_rationale="PMMA (acrylic) has superior optical clarity "
        "(92% transmission) and UV resistance vs PC. "
        "PC is tougher but yellows with UV exposure.",
    ),
    # ── 17. Drive shaft (high-strength steel, torsion) ──────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-017",
        title="Automotive driveshaft — torsional strength + stiffness",
        description="Propeller shaft for RWD vehicle. "
        "Ultimate strength required >400 MPa. High modulus preferred for torsional rigidity.",
        requirements={
        ...
        expected_rejections=[],
        best_candidate="aisi_4340_qt",
        ranking_rationale="4340 steel best combination of stiffness (205 GPa modulus) and strength (1080 UTS). "
        "Aluminum's low modulus (72 GPa) poor for torsional rigidity. "
        "Titanium passes but expensive for automotive.",
    ),
    # ── 18. Underwater housing (316L stainless, marine corrosion) ───────────
    GoldenSelectionProblem(
        id="MATE-GS-018",
        title="ROV pressure housing — deep-sea corrosion",
        description="Remotely operated vehicle housing for 300m depth. "
        "Yield >250 MPa, seawater corrosion resistance, strength-to-weight.",
        requirements={
        ...
        expected_rejections=[],
        best_candidate="ti_6al4v_annealed",
        ranking_rationale="Ti-6Al-4V best for seawater — immune to chloride corrosion, "
        "best specific strength. 316L excellent corrosion but heavier. "
        "6061 needs anodizing + coating which adds maintenance.",
    ),
    ...
    GoldenSelectionProblem(
        ...
        expected_rejections=[],
        best_candidate="peek",
        ranking_rationale="PEEK has better creep resistance and higher tensile "
        "(100 MPa) at elevated temp vs PTFE (31 MPa). "
        "PTFE has wider chemical resistance but lower strength.",
    ),
    # ── 20. Impact-absorbing bumper (polymer toughness) ─────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-020",
        title="Bumper cover — impact absorption, thermoplastic",
        description="Automotive bumper fascia. "
        "Needs tensile >25 MPa, high impact toughness, good moldability, reasonable cost.",
        requirements={
        ...
        expected_rejections=[("abs", "insufficient_context")],
        best_candidate="pc",
        ranking_rationale="PC has excellent impact toughness at low temp. "
        "HDPE also tough but lower stiffness. ABS insufficient_context. "
        "PC margin at 1.6 vs HDPE 0.2.",
    ),
    # ── 21. Turbine blade (nickel superalloy, hot-strength) ─────────────────
    GoldenSelectionProblem(
        id="MATE-GS-021",
        title="Gas turbine blade — high-temperature creep resistance",
        description="First-stage turbine blade. "
        "Must retain yield >300 MPa at 800C+. Creep and oxidation resistance are critical.",
        requirements={
        ...
        ranking_rationale="Inconel 718 is the only material here rated for sustained >700C "
        "with high creep strength. 4340 and Ti-6Al-4V lose strength above ~500C. "
        "Margin: Inconel 2.67.",
    ),
    ...
    GoldenSelectionProblem(
        ...
        ranking_rationale="CFRP has specific stiffness (85.4 vs 25.5 for Al, 25.7 for Ti) — "
        "best stiffness per gram. Margin 5.0 vs 1.01 vs 2.52. "
        "Dominant for high-end frames.",
    ),
    # ── 23. Food processing equipment (316L, cleanability) ──────────────────
    GoldenSelectionProblem(
        id="MATE-GS-023",
        title="Food-grade mixing vessel — sanitary design",
        description="Mixing vessel for dairy processing. Yield >200 MPa. "
        "CIP (clean-in-place) compatible, 3-A sanitary standards, "
        "corrosion resistance to organic acids.",
        requirements={
        ...
        ranking_rationale="316L has Mo for pitting resistance vs organic acids (dairy). "
        "304 is cheaper but susceptible to pitting from chlorides in cleaning chemicals. "
        "316L is food-industry standard.",
    ),
    # ── 24. Electronics heat sink (copper vs aluminum, thermal) ─────────────
    GoldenSelectionProblem(
        id="MATE-GS-024",
        title="CPU heat sink — thermal management",
        description="Extruded fin heat sink for a 150W processor. "
        "Yield is not the driver; thermal conductivity is primary. "
        "Yield >50 MPa to hold shape.",
        requirements={
        ...
        expected_rejections=[],
        best_candidate="c11000_copper",
        ranking_rationale="Copper has 2x thermal conductivity of aluminum "
        "(400 W/mK vs 200). Both easily pass yield requirement. "
        "Copper is standard for high-performance heat sinks.",
    ),
    # ── 25. Ballistic armor (ceramic, extreme compressive) ──────────────────
    GoldenSelectionProblem(
        id="MATE-GS-025",
        title="Body armor plate — ballistic impact",
        description="Hard armor insert plate. "
        "Must resist compressive stress >2000 MPa at the impact point. "
        "Hardness, fracture toughness for multi-hit capability.",
        requirements={
        ...
        expected_rejections=[],
        best_candidate="wc_co_6pct",
        ranking_rationale="WC-Co margin 1.75 (5500 vs 2000). "
        "Alumina margin 0.05 (2100 vs 2000) — survives but at razor-thin margin. "
        "WC-Co dominant for multi-hit capability.",
    ),
    # ── 26. Crane hook (forged steel, ultimate strength) ────────────────────
    GoldenSelectionProblem(
        id="MATE-GS-026",
        title="Crane hook — lifting gear, ultimate strength",
        description="Forged crane hook rated 10 tonnes. "
        "Ultimate tensile strength >500 MPa required. Ductility for overload indication.",
        requirements={
        ...
        expected_rejections=[],
        best_candidate="aisi_4340_qt",
        ranking_rationale="4340 ultimate 1080 MPa (margin 1.16) vs 1045 565 MPa (margin 0.13). "
        "4340 has far superior ultimate strength and fracture toughness for lifting gear.",
    ),
    # ── 27. Dental implant (titanium, small-scale biocompatible) ────────────
    GoldenSelectionProblem(
        id="MATE-GS-027",
        title="Dental implant screw — small-diameter biocompatible",
        description="Endosseous dental implant. Small diameter (3.5mm), "
        "high cyclic bending loads. Yield >400 MPa needed. Biocompatibility required.",
        requirements={
        ...
        expected_rejections=[("ss_316l_annealed", "yield_strength 290")],
        best_candidate="ti_6al4v_annealed",
        ranking_rationale="Ti with margin 1.20 (880 vs 400). "
        "316L rejected — yield 290 < 400. "
        "Ti is also the standard for dental implants due to osseointegration.",
    ),
    # ── 28. Precision instrument base (low CTE, dimensional stability) ──────
    GoldenSelectionProblem(
        id="MATE-GS-028",
        title="Optical bench — dimensional stability",
        description="Laser interferometer baseplate. "
        "Loads are low (tensile 50 MPa). "
        "Dimensional stability over temperature swings is the key constraint.",
        requirements={
        ...
        best_candidate="aisi_1045",
        ranking_rationale="Alumina has compressive_strength only, no tensile/yield → rejected. "
        "Steel has lower CTE (12e-6/K) than aluminum (23e-6/K), "
        "so better dimensional stability. Based on yield margin: steel=5.2, al=4.5.",
    ),
    # ── 29. Oil & gas downhole tool (Inconel, H2S + high strength) ──────────
    GoldenSelectionProblem(
        id="MATE-GS-029",
        title="Downhole logging tool housing — sour service",
        description="Pressure housing for downhole logging in sour gas (H2S) wells. "
        "Yield >600 MPa, NACE MR0175 compliance for sulfide stress cracking.",
        requirements={
        ...
        expected_rejections=[("ss_316l_annealed", "yield_strength 290")],
        best_candidate="inconel_718",
        ranking_rationale="Inconel 718 is NACE MR0175 compliant for sour service. "
        "4340 not rated for H2S (sulfide stress cracking risk). "
        "316L rejected on yield. Inconel margin 0.83 (1100 vs 600).",
    ),
    # ── 30. Consumer product handle (polymer, ergonomic, low load) ──────────
    GoldenSelectionProblem(
        id="MATE-GS-030",
        title="Power tool handle — overmolded grip",
        description="Ergonomic handle for cordless drill. "
        "Tensile >25 MPa, good grip feel, overmolding compatibility, "
        ...
        expected_rejections=[("abs", "insufficient_context")],
        best_candidate="pa66_gf30",
        ranking_rationale="PA66-GF30 highest margin (6.2 vs 0.2 for HDPE). "
        "Glass-filled nylon offers better rigidity, oil resistance, and overmolding "
        "compatibility than HDPE. HDPE too flexible for this application.",
    ),
]
