...
def _run_plugin(ts_code, env_override=None, cwd=None, timeout=15):
    ...
    try:
        ...
        env["GLUDD_HOT_MODULE_PREFIX"] = f"/tmp/gludd-session-e2e-no-hot-{os.getpid()}-"
        if env_override:
...
def _make_state_file(state: dict | None = None) -> str:
    ...
    fpath = f"/tmp/gludd-session-e2e-{os.getpid()}-{worker}-{_state_counter}.json"
    if state is None:
...
def test_primed_with_effective_min_allows_mutation(tmp_path):
    """With the hard 10-dispatch floor, readsTrue + dispatches=10 allows edit."""
    small_state = {
        "started_at": int(time.time() * 1000) - 60_000,
        "readsDone": True,
        "dispatches": 10,
...
def test_stale_session_with_timegate_reset_blocks_after_crash_recovery(tmp_path):
    """Long-idle cross-session state resets to fresh and blocks mutation."""
    old_state = {
    ...
    assert ok, f"stderr: {stderr[:400]}"
    assert "BLOCKED" in stdout, (
        f"Stale session must reset to fresh and block, got: {stdout[:200]}"
    )
