...
class TestCliCommandParsing:
    ...
    def test_daemon_all_flags(self):
        args = self._parse(["daemon", "--host", "0.0.0.0", "--port", "9090",
                            "--log-level", "debug", "--tick-interval", "3.0",
                            "--workers", "4", "--project", "proj-1"])
    ...
    def test_add_all_flags(self):
        args = self._parse([
            "add", "Deploy model", "--queue", "gpu", "--priority", "high",
            "--work-type", "deploy", "--description", "Deploy llama to AWS",
            "--project", "p42", "--daemon-url", "http://0.0.0.0:9999",
        ])
    ...
    def test_list_filter_flags(self):
        args = self._parse(["list", "--queue", "core", "--status", "pending",
                            "--project", "p1", "--daemon-url", "http://h:1"])
        assert args.queue == "core"
    ...
    def test_models_searx_search(self):
        args = self._parse(["models", "searx-search", "gpt", "--source", "github",
                            "--searx-url", "http://s:8080"])
    ...
    def test_models_deploy(self):
        args = self._parse(["models", "deploy", "llama-7b", "--provider", "gcp",
                            "--engine", "llamacpp", "--workload-type", "fine_tuning",
                            "--region", "us-west1", "--gpu-count", "4", "--max-cost", "50.0"])
    ...
    def test_models_add(self):
        args = self._parse(["models", "add", "--model-id", "gpt-4o", "--provider",
    ...
    def test_models_performance(self):
        args = self._parse(["models", "performance", "--service", "openai",
                            "--task-type", "code"])
        assert args.service == "openai"
        assert args.task_type == "code"

    def test_models_ranking(self):
        args = self._parse(["models", "ranking", "--task-type", "code",
                            "--strategy", "cheapest"])
        assert args.task_type == "code"
    ...
    def test_models_router_set(self):
        args = self._parse(["models", "router-set", "--task-type", "code",
                            "--strategy", "fastest"])
        assert args.task_type == "code"
    ...
    def test_local_serve(self):
        args = self._parse(["local-serve", "--model", "llama-7b", "--engine",
                            "llamacpp", "--port", "9999", "--gpu-layers", "32",
                            "--context-size", "8192"])
    ...
    def test_project_add(self):
        args = self._parse(["project", "add", "myproj", "--repo-url", "https://g",
                            "--workspace-path", "/ws", "--weight", "50.0",
    ...
    def test_compute_register(self):
        args = self._parse(["compute", "register", "--id", "ep1", "--url",
                            "http://ep:8000", "--model", "llama", "--max-concurrent", "4"])
        assert args.id == "ep1"
    ...
    def test_compute_launch(self):
        args = self._parse(["compute", "launch", "--provider", "aws", "--gpu",
                            "a100_80", "--model", "llama-70b", "--region",
                            "us-east-1", "--gpu-count", "2", "--max-cost", "25.0",
                            "--no-spot", "--engine", "vllm", "--workload-type",
                            "batch_inference", "--deploy-type", "containerapp"])
    ...
    def test_chat_all_flags(self):
        args = self._parse(["chat", "--model", "openai/gpt-4o", "--system-prompt",
                            "You are helpful", "--history", "/tmp/h.json",
                            "--resume", "--save-interval", "10", "--stream",
                            "--max-context", "32768", "--export", "md",
                            "--export-output", "/tmp/out.md"])
    ...
    def test_ansible_install(self):
        args = self._parse(["ansible", "install", "geerlingguy.nginx",
                            "--type", "role"])
    ...
    def test_integrity_approve(self):
        args = self._parse(["integrity", "approve", "/etc/hosts", "--reason",
                            "expected", "--signer", "ops"])
    ...
    def test_integrity_reject(self):
        args = self._parse(["integrity", "reject", "/etc/shadow",
                            "--reason", "unauthorized"])
    ...
    def test_hooks_register(self):
        args = self._parse(["hooks", "register", "--event", "todo.created",
    ...
    def test_code_graph(self):
        args = self._parse(["code", "graph", "--source", "cli.py",
                            "--language", "python"])
        assert args.source == "cli.py"
        assert args.language == "python"

    def test_code_search(self):
        args = self._parse(["code", "search", "build_parser",
                            "--language", "python"])
    ...
    def test_slurm_submit(self):
        args = self._parse(["slurm", "submit", "--command", "python train.py",
                            "--job-name", "train1", "--partition", "gpu",
                            "--cpus-per-task", "8", "--gpus", "2",
                            "--memory", "32G", "--time-limit", "04:00:00"])
    ...
    def test_connectors_query(self):
        args = self._parse(["connectors", "query", "prometheus",
                            "--spec", '{"query": "up"}'])
    ...
    def test_login_service(self):
        args = self._parse(["login", "github", "--store", "openbao",
                            "--timeout", "60"])
    ...
    def test_onboard(self):
        args = self._parse(["onboard", "aws", "--dry-run", "--region",
                            "us-east-1", "--role-arn", "arn:aws:iam::x"])
        assert args.provider == "aws"
    ...
    def test_make(self):
        args = self._parse(["make", "lint", "--cwd", "/tmp", "--timeout", "30",
                            "--env", "KEY=V", "--stream"])
    ...
    def test_test_background_launch(self):
        args = self._parse(["test", "background", "launch", "test_x.py",
                            "--wait"])
    ...
    def test_test_background_kill(self):
        args = self._parse(["test", "background", "kill", "test_x.py",
                            "--force"])
        assert args.testfile == "test_x.py"
    ...
    def test_test_smoke(self):
        args = self._parse(["test", "smoke", "aws", "ec2-a100", "--live",
                            "--json", "--timeout", "5.0"])
        assert args.provider == "aws"
...
class TestCliSubcommandRouting:
    ...
    def test_all_top_level_leaf_commands_registered(self):
        ...
        expected_leaves = {
            "daemon", "add", "status", "list", "log-level", "deployments",
            "version", "health", "local-serve", "scores", "leaderboard",
            "preflight", "tui", "reload", "chat", "help", "login", "onboard",
            "make",
        }
    ...
    def test_all_group_commands_in_subcommand_map(self):
        ...
        expected_groups = {
            "models", "worktree", "project", "config", "mcp", "skills",
            "compute", "hooks", "workers", "agents",
            "metrics", "templates", "playbooks", "code", "slurm",
            "connectors", "test",
        }
...
class TestAnsibleRunnerPaths:
    ...
    def test_version_symlink_activation(self, tmp_path):
        ...
        root, cleanup = ap.activate_collection_version(
            base, namespace="general_ludd", collection="agent", version="0.1.0",
        )
        assert root.exists()
        assert cleanup is not None
...
class TestAnsibleConversion:
    ...
    def test_normalize_role_output_unicode_analyze(self):
        from general_ludd.ansible.runner import _normalize_role_output
        raw = _normalize_role_output("unicode_analyze", {
            "input_length": 5,
...
class TestTerraformTfvars:
    ...
    def test_build_tfvars_with_deployment_profile(self):
        from general_ludd.infra.terraform import TerraformGenerator
        gen = TerraformGenerator()
        cfg = self._make_config(deployment_profile={
            "tensor_parallel": 4,
            "enforce_eager": True,
            "quantization": "awq",
        })
...
class TestComputeConfig:
    ...
    def test_model_name_validation(self):
        from general_ludd.infra.compute import ComputeConfig, ComputeProvider, GPUType
        valid = ["meta-llama/Llama-3.2-1B", "ghcr.io/org/repo:tag",
                 "ghcr.io/org/repo@sha256:abc123", "llama-7b"]
        for name in valid:
    ...
    def test_workload_type_validation(self):
        from general_ludd.infra.compute import ComputeConfig, ComputeProvider, GPUType
        valid = ["batch_inference", "realtime_api", "fine_tuning",
    ...
    def test_allowed_cidr_validation(self):
        ...
        for cidr in valid:
            cfg = ComputeConfig(provider=ComputeProvider.AWS, gpu_type=GPUType.T4,
                               allowed_cidr=cidr, model_name="x")
