...
Covers:
  ...
  6. GLUDD_STOP_ENFORCE=0 cannot bypass the fundamental pending-work block
...
# ═══════════════════════════════════════════════════════════════════════════════
# 6: GLUDD_STOP_ENFORCE=0 cannot bypass pending work
# ═══════════════════════════════════════════════════════════════════════════════


def test_env_disable_does_not_bypass_pending_work(tmp_path: Path):
    """GLUDD_STOP_ENFORCE=0 cannot disable the fundamental pending-work gate."""
    (tmp_path / "TASKS.md").write_text("- [ ] pending work exists\n")

    result = _invoke_text_complete(
        tmp_path,
        "This message must remain blocked while pending work exists.",
        env_override={"GLUDD_STOP_ENFORCE": "0"},
    )
    assert result is not None
    assert result.get("output_text") == ""
    assert "PENDING WORK EXISTS" in result.get("result_text", "")
