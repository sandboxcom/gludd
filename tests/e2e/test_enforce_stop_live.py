...
def _run_plugin(
    ...
    try:
        ...
        state_id = f"{os.getpid()}-{n}"
        env["GLUDD_STOP_TEXT_COMPLETE_COUNT"] = (
            f"/tmp/gludd-stop-text-complete-count-e2e-live-{state_id}.json"
        )
        env["GLUDD_STOP_STATE_FILE"] = (
            f"/tmp/gludd-stop-state-e2e-live-{state_id}.json"
        )
        env["GLUDD_PERSIST_STOP_BLOCK_FILE"] = (
            f"/tmp/gludd-persist-stop-block-e2e-live-{state_id}.json"
        )
        env["GLUDD_POST_RESULTS_STATE_FILE"] = (
            f"/tmp/gludd-post-results-state-e2e-live-{state_id}.json"
        )
        env["GLUDD_TEXT_ONLY_STATE_FILE"] = (
            f"/tmp/gludd-text-only-state-e2e-live-{state_id}.json"
        )
        env["GLUDD_STOP_TOOL_COUNTS_FILE"] = (
            f"/tmp/gludd-stop-tool-counts-e2e-live-{state_id}.json"
        )
        env["GLUDD_STREAK_FILE"] = (
            f"/tmp/gludd-tool-streak-e2e-live-{state_id}.json"
        )
        env["GLUDD_DISENGAGE_PATH"] = (
            f"/tmp/gludd-watchdog-disengage-e2e-live-{state_id}.json"
        )
        env["GLUDD_BLOCK_COUNTER_FILE"] = (
            f"/tmp/gludd-stop-block-counter-e2e-live-{state_id}.json"
        )
        env["GLUDD_WATCHDOG_CI_FILE"] = (
            f"/tmp/gludd-watchdog-ci-e2e-live-{state_id}.json"
        )
        env["GLUDD_RELEASE_COMPLETENESS_FILE"] = (
            f"/tmp/gludd-release-completeness-e2e-live-{state_id}.json"
        )
        env["GLUDD_LAST_TEST_RESULT_FILE"] = (
            f"/tmp/gludd-last-test-result-e2e-live-{state_id}.json"
        )
        env["GLUDD_MULTITASK_STATE_FILE"] = (
            f"/tmp/gludd-multitask-state-e2e-live-{state_id}.json"
        )
        if env_override:
...
def test_env_disable_bypasses_enforcement(tmp_path: Path):
    ...
    assert result.get("result_text") == (
        ...
        f"GLUDD_STOP_ENFORCE=0 must pass through unchanged. Got: {result}"
    )
