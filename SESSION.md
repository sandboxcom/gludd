## PRIMARY OBJECTIVE: IN PROGRESS — v0.1.0-beta.4 prepared; release-cut blocked on CI green. Session 85: HEAD `a7816ed3` on `development`. Round 12 verdicts: build 31959156193 FAILED with 6 classes (opencode-live binary FileNotFoundError ×5 + molecule-timeout pin 45>30) — ALL fixed locally; molecule 31959156421 CANCELLED at 45m (the infinite retry loop — root-caused and fixed `de25ad544`). Round 13 (31961002733/31961002734) in flight with the retry fix. Local molecule local_game_gen GREEN (deterministic 3-attempt rejection, artifact removed, REJECTED surfaced, verify rejection path). Three commits ride round 14: e0d56343 (fallback switch gated on configured fallbacks), 26a6a242 (opencode-live gates + timeout pin 45), a7816ed3 (TUI gate). Next: round-13 verdicts → push round 14 → CI green → `make development-merge-to-master` → `make release-cut TAG='v0.1.0-beta.4'`.
