## PRIMARY OBJECTIVE: IN PROGRESS — v0.1.0-beta.4 prepared; release-cut blocked on CI green. Session 85: HEAD `bd54f13d` on `development`. Round 7 verdicts: molecule 31923822224 FAILED (only local_game_gen — invalid include_role vars option, fixed); build 31923822261 FAILED (unit-2 make-target-contract help pin vs internal-target convention conflict — fixed; ALL other unit shards GREEN). Round 8 pushed + verified (`VERIFIED development@bd54f13d4`): real model fallback (download fallback gguf + server restart + corrective prompt), cross-task weight-DB reuse (global per-model rankings via get_summary; router picks historical best for never-seen tasks; live e2e ungated), acceptance engine rejects import-time FS side effects (subprocess probe FS-diff + cleanup), contract check exempts internal targets from help, converge vars back at task level. KNOWN REMAINING (unchanged rounds 5-7, pre-existing, NOT touched this session): "other"-shard e2e/security cluster (~38 tests): opencode-live ×2, sandbox timeout, ansible templating, binary version, connector e2e (SearX/Monday/procsys/Nomad ×4/Journald), delegate_e2e, enforce_stop_live, event_loop_workflows, game_building_local, governance, iam ×2, local_serve ×4, mac_memory, model_download, pricing ×5, SSL/HSM, retrieval, ci_workflow, daemon_auth ×2, sast ×3 (B102 exec game_gen/acceptance, B106 sts/nat_traversal, B413 salsa20, B324 md5, B602). Next: round-8 verdicts → dedicated wave for the "other"-shard cluster → CI green → `make development-merge-to-master` → `make release-cut TAG='v0.1.0-beta.4'`.
