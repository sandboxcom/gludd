## PRIMARY OBJECTIVE: IN PROGRESS — v0.1.0-beta.4 prepared; release-cut blocked on CI green. Session 85: HEAD `c2b4b05c` on `development`. Round 5: molecule 31898709653 FAILED — local_game_gen retry self-include infinite Jinja recursion (fixed `c2b4b05c` by dropping the self-referential `_attempt` vars on `include_tasks`); build 31898709411 still in flight. Round 4 fixes (molecule /v1/models probe, root molecule.yml, unit-2 interpreter/graph/scenario/TaskRole/perf) all landed in round 5. Next: build verdict → push round-6 (role fix + docs) → CI green → `make development-merge-to-master` → `make release-cut TAG='v0.1.0-beta.4'`.
