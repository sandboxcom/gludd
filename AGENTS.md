...
tracked in `config/make_target_contract.json`. Run
`make check-make-target-contract` after changing a target, and run its documented
behavioral example before claiming the target works. If no suitable target exists,
add and test one first.
