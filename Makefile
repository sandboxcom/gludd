...
# Push a specific SHA to a remote branch via SSH (needs sandboxcom_github_rsa in main checkout).
# Usage: make git-push-ssh SHA=<sha> TARGET=<branch>
git-push-ssh:
	@[ -n "$(SHA)" ] || { echo "Usage: make git-push-ssh SHA=<sha> TARGET=<branch>"; exit 1; }
